...
extern void ifx_vr9_mdio_write(unsigned char phyaddr, unsigned char phyreg, unsigned short data);
extern unsigned short ifx_vr9_mdio_read(unsigned char phyaddr, unsigned char phyreg);

/* GPHY MMD register accessing constraint. */
#define MDC_MDIO_WRITE( preLen, phy, reg, data )		ifx_vr9_mdio_write( phy, reg, data )
#define MDC_MDIO_READ(  preLen, phy, reg, pdata )		do { *(pdata)=ifx_vr9_mdio_read( phy, reg ); } while (0)

#else
extern void ifx_ltq_write_mdio(unsigned int phyAddr, unsigned int regAddr,unsigned int data );
extern unsigned short ifx_ltq_read_mdio(unsigned int phyAddr, unsigned int regAddr );

#define MDC_MDIO_WRITE( preLen, phy, reg, data )		ifx_ltq_write_mdio( phy, reg, data )
#define MDC_MDIO_READ(  preLen, phy, reg, pdata )		do { *(pdata)=ifx_ltq_read_mdio( phy, reg ); } while (0)
#endif
