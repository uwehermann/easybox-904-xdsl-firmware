...
printErr(char const *str)
{
    ...
    SetPPPSystemLog(LOG_LEVEL_DEFAULT_STR, LOG_MESSAGE_TYPE_DEFAULT_STR, "[ppp] %s", str);
}
#endif
...
clampMSS(PPPoEPacket *packet, char const *dir, int clampMss)
{
	...
	SetPPPSystemLog(LOG_LEVEL_DEFAULT_STR, LOG_MESSAGE_TYPE_DEFAULT_STR, "[ppp] Bad TCP checksum %x", (unsigned int) csum);
	...
	case 2:
		...
		SetPPPSystemLog(LOG_LEVEL_DEFAULT_STR, LOG_MESSAGE_TYPE_DEFAULT_STR,
		       "[ppp] Bogus length for MSS option (%u) from %u.%u.%u.%u",
		       (unsigned int) opt[1],
		       (unsigned int) ipHdr[12],
		       (unsigned int) ipHdr[13],
		       (unsigned int) ipHdr[14],
	...
	default:
		...
		syslog(LOG_ERR,
		       ...
		       (unsigned int) ipHdr[15]);
		SetPPPSystemLog(LOG_LEVEL_DEFAULT_STR, LOG_MESSAGE_TYPE_DEFAULT_STR,
		       "[ppp] Bogus TCP option length (%u) from %u.%u.%u.%u",
		       (unsigned int) opt[1],
		       (unsigned int) ipHdr[12],
		       (unsigned int) ipHdr[13],
		       (unsigned int) ipHdr[14],
	     ...
	     void *extra)
{
	...
	SetPPPSystemLog(LOG_LEVEL_DEFAULT_STR, LOG_MESSAGE_TYPE_DEFAULT_STR, "[ppp] PADT: Service-Name-Error: %.*s", (int) len, data);
	fprintf(stderr, "PADT: Service-Name-Error: %.*s\n", (int) len, data);
	break;
    case TAG_AC_SYSTEM_ERROR:
	syslog(LOG_ERR, "PADT: System-Error: %.*s", (int) len, data);
	SetPPPSystemLog(LOG_LEVEL_DEFAULT_STR, LOG_MESSAGE_TYPE_DEFAULT_STR, "[ppp] PADT: System-Error: %.*s", (int) len, data);
	fprintf(stderr, "PADT: System-Error: %.*s\n", (int) len, data);
	break;
    case TAG_GENERIC_ERROR:
	syslog(LOG_ERR, "PADT: Generic-Error: %.*s", (int) len, data);
	SetPPPSystemLog(LOG_LEVEL_DEFAULT_STR, LOG_MESSAGE_TYPE_DEFAULT_STR, "[ppp] PADT: Generic-Error: %.*s", (int) len, data);
