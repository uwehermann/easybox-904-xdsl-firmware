...
//Simon@2011/11/07, Big endian cpu process little endian file.
	...
	pHead->BiSize = swap32(pHead->BiSize);
	pHead->BiWidth = swap32(pHead->BiWidth);
	pHead->BiHeight = swap32(pHead->BiHeight);
	pHead->BiPlanes = swap16(pHead->BiPlanes);
	pHead->BiBitCount = swap16(pHead->BiBitCount);
	pHead->BiCompression = swap32(pHead->BiCompression);
	pHead->BiSizeImage = swap32(pHead->BiSizeImage);
	pHead->BiXpelsPerMeter = swap32(pHead->BiXpelsPerMeter);
	pHead->BiYpelsPerMeter = swap32(pHead->BiYpelsPerMeter);
	pHead->BiClrUsed = swap32(pHead->BiClrUsed);
	pHead->BiClrImportant = swap32(pHead->BiClrImportant);
#endif

#if 0
	printf("<%15s@%30s:%5d>: pHead->BiSize = 0x%x\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiSize);
	printf("<%15s@%30s:%5d>: pHead->BiWidth = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiWidth);
	printf("<%15s@%30s:%5d>: pHead->BiHeight = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiHeight);
	printf("<%15s@%30s:%5d>: pHead->BiPlanes = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiPlanes);
	printf("<%15s@%30s:%5d>: pHead->BiBitCount = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiBitCount);
	printf("<%15s@%30s:%5d>: pHead->BiCompression = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiCompression);
	printf("<%15s@%30s:%5d>: pHead->BiSizeImage = 0x%x\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiSizeImage);
	//printf("<%15s@%30s:%5d>: pHead->BiXpelsPerMeter = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiXpelsPerMeter);
	//printf("<%15s@%30s:%5d>: pHead->BiYpelsPerMeter = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiYpelsPerMeter);
	printf("<%15s@%30s:%5d>: pHead->BiClrUsed = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiClrUsed);
	printf("<%15s@%30s:%5d>: pHead->BiClrImportant = %d\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pHead->BiClrImportant);
#endif
	...
	do {
		...
		if (bitdepth > 8 && bitdepth != 24) {
			printf("<%15s@%30s:%5d>: Error: bits per pixel must be 1, 4, 8 or 24\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__ );
			break;
		}
		...
		do {
			...
			pImageHdr->imagebits = (MWUCHAR *) malloc(sizeof(MWUCHAR) * linesize * cy);
			...
			//Simon@2011/11/07, Change Default Transparent Color
			pImageHdr->transcolor = 0x1234; //-1L;
			retV = pImageHdr;
		} while (0);
	} while (0);
