...
//Simon@2011/11/16, Don't queue paint msgs
#ifdef DO_NOT_QUEUE_PAINT
#define PAINTONCE	0	/* =1 to queue paint msgs only once*/
#else
...
//Simon@2011/12/05, force clip to z-parent
#ifndef FORCE_CLIP_TO_Z_PARENT
	...
	//Simon@2011/12/01, Don't do sibling cliping
	...
	memset(wp->szTitle, 0, titLen + 1);	//initialization
...
MwDestroyWindow(HWND hwnd,BOOL bSendMsg)
{
	...
	printf("<%15s@%30s:%5d>: Destroy 0x%x from root child 0x%x\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, wp, rootwp->children);
	/*
	 * Unmap the window.
	 */
	if (wp->unmapcount == 0)
