...
//Simon@2011/12/xx, Fix Free Mru Resource Bug
#ifdef FIX_FREE_MRU_RESOURCE_BUG
		while(obj){
			prev = obj;
			obj = obj->next;
			if (prev->cLock <= 0) {
				//relink
				mruResources->next = obj;
				//free resource of prev
				if (prev->pData) free(prev->pData);
				free(prev);
				mruResCount--;
				ndel++;
			}
...
resGetDlgTemplExtra(PMWDLGTEMPLATE pDlg, PMWDLGTEMPLEXTRA pDlgExtra)
{
	...
	pDlgExtra->pItemsExtra = malloc(sizeof(MWDLGITEMTEMPLEXTRA) * pDlg->cdit);
