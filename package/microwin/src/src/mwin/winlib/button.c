...
static void WINAPI 	DrawPushButton(HWND hwnd,HDC hDCwParam,UINT wEnumState,DWORD dwStyle);
...
void Button_SetBackground( UINT IDNormal, UINT IDFocus, UINT IDPressed)
{
	m_ButtonBgNormal = IDNormal;
	m_ButtonBgFocus = IDFocus;
	m_ButtonBgPressed= IDPressed;
}

void Button_SetTextColor( COLORREF crNormal, COLORREF crFocus, COLORREF crPressed)
{
	m_ButtonTextColorNormal = crNormal;
	m_ButtonTextColorFocus = crFocus;
	m_ButtonTextColorpressed = crPressed;	
}
...
HWND		hwndNewFocus)
{
	...
	if( (hwnd->szTitle[0] != 0) && ((hwnd->style & (BS_FLAT/*|BS_NOFOCUSRECT*/)) == 0) )
		...
		//DrawFocusRect ( hdc, &rect );//mike no focus should be marked


		if(hwnd->style & BS_BITMAP)
		{
			//printf("%s(%d): m_ButtonBgNormal=%d\n",__FUNCTION__,__LINE__,m_ButtonBgNormal);
			himg = resLoadBitmap(hwnd->hInstance, MAKEINTRESOURCE(m_ButtonBgNormal));
			if (himg != NULL) 
			{
				DrawDIB(hdc, rc.left, rc.top, himg);
				resFreeBitmap(himg);
			}
		}
		...
		if( (GET_PBSTATE( hwnd) & PUSH_DISABLED) || !IsWindowEnabled(hwnd) )
			hOldColor = SetTextColor( hdc,
				GetSysColor( COLOR_GRAYTEXT));
		else
			hOldColor = SetTextColor( hdc, m_ButtonTextColorNormal); 	
		...
		//DrawText( hdc, buf, -1, &rect,DT_LEFT | DT_TOP );
		dwStyle = GetWindowLong(hwnd, GWL_STYLE); 	
		switch((int)(dwStyle & (BS_LEFT|BS_CENTER|BS_RIGHT))) {
		case BS_CENTER:
		default:
			DrawText( hdc, buf, -1, &rect,DT_CENTER |DT_TOP| DT_WORDBREAK);
			break;
		case BS_LEFT:
			DrawText( hdc, buf, -1, &rect,DT_LEFT|DT_TOP| DT_WORDBREAK);
			break;
		case BS_RIGHT:
			DrawText( hdc, buf, -1, &rect,DT_RIGHT|DT_TOP| DT_WORDBREAK);
			break;
		}	
...
HDC		hDCwParam)
{
	...
	//printf("cenButton_OnPaint dwStyle = %d\n",(dwStyle & 0x0f));
	case BS_GROUPBOX:
...
#define uiWidthShadow	2
	...
	//printf("GetWindowText = %s",buf);
	GetClientRect( hwnd, &rectClient );

	//printf("%s(%d) : rectClient left=%d righ=%d top=%d bottom=%d\n",__FUNCTION__,__LINE__,rectClient.left,rectClient.right,rectClient.top,rectClient.bottom);
   ...
   if (dwStyle & BS_PUSHLIKE)
	...
	//printf("DrawPushButton style = 0x%x\n",(dwStyle & 0x0f));
	//printf("DrawPushButton wEnumState = 0x%x\n",wEnumState);
	...
	case BS_DEFPUSHBUTTON:
		if( (wEnumState & PBS_FOCUSDOWN) || (wEnumState & PBS_CHECKED)) {
			...
			//printf("%s(%d): m_ButtonBgPressed=%d\n",__FUNCTION__,__LINE__,m_ButtonBgPressed);
			himg = resLoadBitmap(hwnd->hInstance, MAKEINTRESOURCE(m_ButtonBgPressed));
			if (himg != NULL) 
			{
				DrawDIB(hdc, rc.left, rc.top, himg);
				resFreeBitmap(himg);
			...
			//DrawDIB(hdc, rc.left, rc.top, resLoadBitmap(hwnd->hInstance, MAKEINTRESOURCE(m_ButtonBgPressed)));
			...
			*/
			...
			{				
				if( (GetFocus() == hwnd) )
				{					
					//printf("%s(%d): m_ButtonBgFocus=%d\n",__FUNCTION__,__LINE__,m_ButtonBgFocus);
					himg = resLoadBitmap(hwnd->hInstance, MAKEINTRESOURCE(m_ButtonBgFocus));
					if (himg != NULL) 
					{
						DrawDIB(hdc, rc.left, rc.top, himg);
						resFreeBitmap(himg);
					}				
					//DrawDIB(hdc, rc.left, rc.top, resLoadBitmap(hwnd->hInstance, MAKEINTRESOURCE(m_ButtonBgFocus)));
				}					
				else
				{			
					//printf("%s(%d): m_ButtonBgNormal=%d\n",__FUNCTION__,__LINE__,m_ButtonBgNormal);
					...
					//DrawDIB(hdc, rc.left, rc.top, resLoadBitmap(hwnd->hInstance, MAKEINTRESOURCE(m_ButtonBgNormal)));
				}
			}
	...
	if(buf[ 0]) {
		...
		*/ 
		...
		if( (wEnumState & PBS_DISABLED) || !IsWindowEnabled(hwnd) )
			//hOldColor = SetTextColor( hdc, GetSysColor( COLOR_GRAYTEXT));
			hOldColor = SetTextColor( hdc, RGB(178,178,178));
		else
			//hOldColor = SetTextColor( hdc,
				//GetSysColor( COLOR_BTNTEXT));
		{
			if( (wEnumState & PBS_FOCUSDOWN) || (wEnumState & PBS_CHECKED)) 
				hOldColor = SetTextColor( hdc, m_ButtonTextColorpressed);				
			else if( (GetFocus() == hwnd) )
				hOldColor = SetTextColor( hdc, m_ButtonTextColorFocus);
			else
				hOldColor = SetTextColor( hdc, m_ButtonTextColorNormal);				
		}
		 

		//printf("DrawPushButton left=%d right=%d top=%d bottom=%d\n",rect.left,rect.right,rect.top,rect.bottom);
...
LPARAM	lParam)
{
	...
	case WM_CHAR:
		  ...
		  //printf("%s(%d) : WM_KEYDOWN WM_CHAR MSG = 0x%x, wParam = 0x%x \n",__FUNCTION__,__LINE__,message,wParam);
		  
	    if((message == WM_CHAR) && ( (char)wParam == ' ' || wParam == VK_RETURN || wParam == VK_MBUTTON))
			{
...
#else
{
		...
		return 1;
	case BM_SETIMAGE:
					//printf("cenButtonWndFn : BM_SETIMAGE\n");
