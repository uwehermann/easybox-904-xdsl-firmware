...
#ifdef COMPILE_MIDDLE_CODE 
	#ifdef LCD_904 
	#include <lcd_904.h>
	#else
	#include <lcd_ili9325d.h>
	#endif
...
//Simon@2011/11/07, Big endian cpu process little endian file. 
	...
	while (obj) {
		...
		if (IsWindowVisible(obj)) {
			if ((dlgCode & DLGC_STATIC) ||
				...
				//printf("<%15s@%30s:%5d>: szTitle = [%c][%c][%c][%c][%c]\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, (unsigned char)pCaption[0], (unsigned char)pCaption[1], (unsigned char)pCaption[2], (unsigned char)pCaption[3], (unsigned char)pCaption[4]);		
				//printf("<%15s@%30s:%5d>: szTitle = [%c][%c][%c][%c][%c]\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, (unsigned char)pCaption[5], (unsigned char)pCaption[6], (unsigned char)pCaption[7], (unsigned char)pCaption[8], (unsigned char)pCaption[9]);		
				LPCTSTR pAccel = strchr(pCaption, '&');

				//Simon@2011/12/14, make &&X works.
...
//Simon@2011/11/21, paint dialog
#ifdef Full_WINDOW_UPDATE
		case WM_PAINT:
		{
			struct hwnd* sibling;
			struct hwnd* children[20];
			int childCount, i;

			if(hWnd->children) children[0] = hWnd->children;
			else break;
			//printf("<%15s@%30s:%5d>: 1st child = 0x%x\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, children[0]);
			childCount=1;
			sibling = children[0]->siblings;
			while(sibling != NULL){
				//printf("<%15s@%30s:%5d>: sibling = 0x%x\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, sibling);
				children[childCount]= sibling;
				sibling = sibling->siblings;
				childCount++;
			}
			//printf("<%15s@%30s:%5d>: dialog %x has %d children\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, hWnd, childCount);			

			for(i=0; i<childCount; i++){
				if(children[i]->style & WS_VISIBLE) SendMessage(children[i], WM_PAINT, 0, 0L);
			}
			break;
		}
#endif
	...
	if (lpMsg->hwnd == hDlg || ISDLGCONTROL(hDlg, lpMsg->hwnd)) {
		...
		case WM_CHAR:
			if ((dlgCode & (DLGC_WANTCHARS)))
				break;
			if ((lpMsg->wParam == '\t') && (dlgCode & DLGC_WANTTAB))
				break;
		...
		while (IsWindow(hDlg) && params->running) {
			if (!PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
				if (bSendIdle) {
					...
					bSendIdle = FALSE;
					//Refresh LCD Screen
					ioctl( iFd, LCD_IOC_REFRESH, &stRegion );
					ioctl( iFd, LCD_IOC_REFRESH, &stRegion );
				}
...
//Simon@2011/11/xx, Do not draw wallpaper
#ifdef DO_NOT_DRAW_WALLPAPER			
			if(msg.message == WM_PAINT && (msg.hwnd == GetDesktopWindow())){ 
				printf("<%15s@%30s:%5d>: skip drawing desktop wallpaper 0x%x\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, GetDesktopWindow());
				continue;
			}	
#endif
...
#ifdef DEBUG_DIALOG				
				printf("<%15s@%30s:%5d>: None Dialog Message %x 0x%x received\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, msg.message, msg.hwnd);
#endif
...
#ifdef DEBUG_DIALOG			
			printf("<%15s@%30s:%5d>: %x 0x%x from (%d,%d) to (%d,%d)\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, msg.message, msg.hwnd, msg.hwnd->winrect.left, msg.hwnd->winrect.top, msg.hwnd->winrect.right, msg.hwnd->winrect.bottom);
#endif
			//Refresh LCD Screen
			//ioctl( iFd, LCD_IOC_REFRESH, &stRegion );
		...
		printf("<%15s@%30s:%5d>: Dialog 0x%x destroed\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, hDlg);
		printf("\n");
		if (hWndParent)
...
{
	HWND hDlg;
	hDlg = CreateDialogParam(hInstance, lpTemplate, hWndParent, lpDialogFunc, lParam);
#ifdef DEBUG_DIALOG
	printf("<%15s@%30s:%5d>: Dilg 0x%x created\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, hDlg);
	printf("\n");
#endif	
	return DIALOG_DoDialogBox (hDlg, hWndParent);
}
...
{
	static const char defAlpha[] = " ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
	SIZE sz;
	HDC hdc;
	HFONT oldFnt;
	BOOL retV = FALSE;
	...
	//if( GetTextMetrics(hdc, &tm) )
	{
		...
		sz.cx = (sz.cx + (sizeof(defAlpha) - 1) / 2) / (sizeof(defAlpha) - 1);
...
HWND WINAPI
CreateDialog(HINSTANCE hInstance, LPCSTR lpTemplate, HWND hWndParent, DLGPROC lpDialogFunc)
{
	return CreateDialogParam(hInstance, lpTemplate, hWndParent, lpDialogFunc, 0);
}

HWND WINAPI
CreateDialogIndirectParam(HINSTANCE hInstance, LPCDLGTEMPLATE lpTemplate, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam)
{
...
//Simon@2011/11/07, Big endian cpu process little endian file.
#ifdef HACK_RES_FILE_ENDIAN_DIFFERENT
			pItem->style = swap32(pItem->style);
			pItem->dwExtendedStyle = swap32(pItem->dwExtendedStyle);
			pItem->id = swap16(pItem->id);
			pItem->x = swap16(pItem->x);
			pItem->y = swap16(pItem->y);
			pItem->cx = swap16(pItem->cx);
			pItem->cy = swap16(pItem->cy);
#endif
			//Verified OK
			//printf("<%15s@%30s:%5d>: pItem->style = %x \n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pItem->style);
			//printf("<%15s@%30s:%5d>: pItem->dwExtendedStyle = %x \n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pItem->dwExtendedStyle);
			//printf("<%15s@%30s:%5d>: pItem->id = %x \n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pItem->id);
			//printf("<%15s@%30s:%5d>: pItem->x = %x \n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pItem->x);
			//printf("<%15s@%30s:%5d>: pItem->y = %x \n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pItem->y);
			//printf("<%15s@%30s:%5d>: pItem->cx = %x \n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pItem->cx);
			//printf("<%15s@%30s:%5d>: pItem->cy = %x \n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, pItem->cy);
...
#ifdef DEBUG_DIALOG			
			printf("<%15s@%30s:%5d>: Ctrl 0x%x created\n", strrchr(__FILE__,'/')+1, __FUNCTION__, __LINE__, hCtrl);
#endif
			if (hCtrl != NULL) {
				if ((hFocus == NULL)
...
HWND WINAPI
CreateDialogParam(HINSTANCE hInstance, LPCTSTR lpTemplate, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam)
{
	PMWDLGTEMPLATE pDlg;
	HWND hRet;
	HGLOBAL hResDlg;
...
//Simon@2011/11/07, Big endian cpu process little endian file.
#ifdef HACK_RES_FILE_ENDIAN_DIFFERENT
		pDlg->style = swap32(pDlg->style);
		pDlg->dwExtendedStyle = swap32(pDlg->dwExtendedStyle);
		pDlg->cdit = swap16(pDlg->cdit);
		pDlg->x = swap16(pDlg->x);
		pDlg->y = swap16(pDlg->y);
		pDlg->cx = swap16(pDlg->cx);
		pDlg->cy = swap16(pDlg->cy);
#endif
