...
#define	ARC_VERSION /*ctc 20091104 modified for windows LPR port*/
...
static char *xmalloc_read_stdin(int *pLen)
{
	// SECURITY:
	size_t max = 4 * 1024; // more than enough for commands!
	char* pRead = xmalloc_reads(STDIN_FILENO, NULL, &max);
	if (pRead && pLen)
		*pLen = (int)max; 
	return pRead;
}
...
int lpd_main(int argc UNUSED_PARAM, char *argv[])
{
 ...
 unsupported_cmd:
	...
	while (1) {
	  ...
	  #ifdef ARC_VERSION
		pContent = strchr(s,'\n');
		if ( (pContent=strchr(s,'\n')) != 0)
		{
			pContent++;
			iContentLen = iReadLen - (int)(pContent - s);
		}
		else
		{
			iContentLen = 0;
...
//			goto bad_fname;
		// get length
	  #ifdef ARC_VERSION
		expected_len = bb_strtoull(s + 1, NULL, 10);
	  #else
		...
		{
			safe_write(fd,pContent,iContentLen);
			expected_len -= iContentLen;
		}
		if (expected_len > 0)
		{
			while (1) {
				...
				if (real_len <= 0) {
					printf("Expected %d but nothing got\n", expected_len);
					goto err_exit;
				}
