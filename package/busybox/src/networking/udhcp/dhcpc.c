...
static void perform_renew(void)
{
	...
	case RELEASED:
		...
		//printf("[perform_renew]: Reset start_time\n");
		break;
	case INIT_SELECTING:
		break;
	}
...
int udhcpc_main(int argc UNUSED_PARAM, char **argv)
{
	...
	static const char udhcpc_longopts[] ALIGN1 =
		...
		"mac-addr\0"       Required_argument "m"
...
/* The rest has variable bit positions, need to be clever */
	...
	opt = getopt32(argv, "c:CV:H:h:F:i:np:qRr:s:T:t:SA:O:of:m"
		USE_FOR_MMU("b")
	...
	if (opt & OPT_m)
	{
		...
		//Convert MAC address string (12 bytes) to hex (6 bytes) 
		{
			memset(mac, 0, 7);		
			j = 0;
			for(i = 0 ; i<13 ; i++)
		...
		client_config.clientid = alloc_dhcp_option(DHCP_CLIENT_ID, mac, 0);
	}
	if (opt & OPT_V)
	...
	state = INIT_SELECTING;
	start_time = time(NULL);
	//printf("[udhcpc_main]: Reset start_time, line %d\n", __LINE__);
	udhcp_run_script(NULL, "deconfig");
	change_listen_mode(LISTEN_RAW);
	packet_num = 0;
	timeout = 0;
 ...
 leasefail:
			...
			case REBINDING:
				...
				state = INIT_SELECTING;
				start_time = time(NULL);
				//printf("[udhcpc_main]: Lease lost, Reset start_time\n");
