...
static int check_trustedgroup(const uid_t uid, const gid_t gid)
{
  ...
  #if 0 /*ctc 20100301 modified to accept root but restrict its home directory*/
    if (uid == (uid_t) 0) {
        return 1;
    }
  #endif
...
void dostor(char *name, const int append, const int autorename)
{
  ...
  #if 1 /*ctc 20110414 modified to do firmware upgrade */
   #define UPGRADE_FILE_PREFIX "upgrade-"
   #define FTP_UPGRADE_FILE_PREFIX "ftp-"
   #define FTP_UPGRADE_FILE_PREFIX2 "ftp-upgrade-"
	if ( name && append == 0
	  && strncmp( name, FTP_UPGRADE_FILE_PREFIX2, sizeof(FTP_UPGRADE_FILE_PREFIX2)-1 )==0 )
	{
...
# endif    
   ...
   {
	char sNewName[128];
	if ( append == 0 && name != NULL
	  && strlen(name) < sizeof(sNewName)-sizeof(FTP_UPGRADE_FILE_PREFIX)
	  && strncmp( name, UPGRADE_FILE_PREFIX, sizeof(UPGRADE_FILE_PREFIX)-1 ) == 0 )
	{
		sprintf( sNewName, "%s%s", FTP_UPGRADE_FILE_PREFIX, name );
		rename( name, sNewName );
	}
   }
  #endif
}
