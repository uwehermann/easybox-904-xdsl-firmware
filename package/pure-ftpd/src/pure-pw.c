...
static int parse_pw_line(char *line, PWInfo * const pwinfo)
{
  ...
  #if 0 /*ctc 20100301 modified to accept root*/
...
static int do_crypt( char* sPasswd )
{
	char	pwd[LINE_MAX];
	char*	crypt;
	...
	if (sPasswd == NULL)
	{
	    printf("Password: ");
	    fflush(stdout);
	    if (fgets(pwd, sizeof(pwd)-1, stdin) == NULL)
	        return PW_ERROR_ENTER_PASSWD_PW_ERROR;
	}
	else
	{
		strncpy( pwd, sPasswd, sizeof(pwd)-1 );
	}

    strip_lf(pwd);
    crypt = best_crypt(pwd);
	if (!crypt)
		return PW_ERROR_ENTER_PASSWD_PW_ERROR;
...
# endif
    ...
    if (strcasecmp(action, "useradd") == 0) {
        ...
        ret = do_list(file);
    } else if (strcasecmp(action, "crypt") == 0) {
        ret = do_crypt( pwinfo.login );
    } else {
