...
//ygchen, 2013/01/27, the below implementation results in failure of local DNS queries if there's ipv6 DNS server in resolv.conf.auto
//so disable it temporarily
#if 0
				if((udpaddr!=NULL) && (start!=NULL))
				{
					if(udpaddr->sa.sa_family == start->addr.sa.sa_family)
					{
						need_forward = 1;
					}
					else                  
				...
				if (need_forward==1 && sendto(fd, (char *)header, plen, 0,
