...
#ifndef ULOGD_PCAP_MAXSZ_DEFAULT
#define ULOGD_PCAP_MAXSZ_DEFAULT	0x200000	/*2MB*/
#endif
...
static int pcap_output(ulog_iret_t *res)
{
	...
	fseek( of, 0, SEEK_END );
	lOffset = ftell( of );
	if (lOffset >= ULOGD_PCAP_MAXSZ_DEFAULT) {
		ulogd_log(ULOGD_ERROR, "pcap file over size\n");
		return 1;
	}
	...
	if (fwrite(GET_VALUE(0).ptr, GET_VALUE(1).ui32, 1, of) != 1) {
		ulogd_log(ULOGD_ERROR, "Error during write: %s\n",
			  strerror(errno));
		return 1;
	}
