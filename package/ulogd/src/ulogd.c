...
#ifndef ULOGD_LOGFILE_MAXSZ_DEFAULT
#define ULOGD_LOGFILE_MAXSZ_DEFAULT	0x40000	/*256KB*/
#endif

/*#define ARCADYAN_DEBUG*/
#ifdef ARCADYAN_DEBUG
  int printfConsole(const char *sFmt, ...);
  void dumpData( char* buffer, int count );
#endif
...
void __ulogd_log(int level, char *file, int line, const char *format, ...)
{
	...
	if (logfile == &syslog_dummy) {
		...
		if (logfile)
		{
			...
			if (lOffset >= ULOGD_LOGFILE_MAXSZ_DEFAULT) {
			  #ifdef ARCADYAN_DEBUG
				printfConsole("ulogd log file over size\n");
			  #endif
				return;
			}
			outfd = logfile;
...
void dumpData( char* buffer, int count )
{
	size_t iTCnt, iCCnt, iCnt, iPos;
	unsigned char printbuf[16];
	char linebuf[80];

	printfConsole("%d bytes received\n",count);

	for (iTCnt=0; iTCnt<count; iTCnt+=16)
	{
		if ((count-iTCnt) >= 16)
			iCCnt = 16;
		else
			iCCnt = count - iTCnt;
		memcpy(printbuf, buffer+iTCnt, iCCnt);
		memset(linebuf,0x20,sizeof(linebuf));
		linebuf[sizeof(linebuf)-1] = '\0';
		for (iCnt=0; iCnt<iCCnt; iCnt++)
		{
			iPos=(iCnt*3);
			linebuf[iPos+0] = '0' + (printbuf[iCnt] >> 4);
			linebuf[iPos+1] = '0' + (printbuf[iCnt] & 0xf);
			linebuf[iPos+2]=' ';
			if(linebuf[iPos+0] > '9')
				linebuf[iPos+0] = 'a' + (linebuf[iPos+0] - '9' - 1);
			if(linebuf[iPos+1] > '9')
				linebuf[iPos+1] = 'a' + (linebuf[iPos+1] - '9' - 1);
			if (printbuf[iCnt] >= 0x20 && printbuf[iCnt] <= 0x7e)
				linebuf[50+iCnt] = printbuf[iCnt];
			else
				linebuf[50+iCnt] = '.';
		}
		printfConsole("%s\n",linebuf);
