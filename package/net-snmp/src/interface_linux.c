                                      ...
                                      u_int load_flags)
{
    ...
    while (fgets(line, sizeof(line), devin)) {
        ...
        *stats++ = 0; /* null terminate name */
		...
		//Upstream bandwidth record in ADSL or VDSL interface
		if((strstr(entry->descr, "nas") != NULL) || (strstr(entry->descr, "ptm") != NULL))
		{
			FILE *fp;
			int speed;
			char line[32];

			system("/usr/sbin/libmapi_dsl_cli dsl_status_get");
			usleep(100);
			fp = fopen("/tmp/xdsl_usDataRate", "r");
			...
			if(speed > 0)
				entry->speed = (speed * 1024);
		}

		//Upstream bandwidth record in PPP interface
		if(strstr(entry->descr, "ppp") != NULL)
		{
			FILE *fp;
			int speed;
			char line[32];
			...
			system("/usr/sbin/libmapi_dsl_cli dsl_status_get");
			usleep(100);
			fp = fopen("/tmp/xdsl_dsDataRate", "r");
