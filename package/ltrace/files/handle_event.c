...
#include "syscallent1.h"
	...
	if (sysnum < 0 || sysnum >= nsyscals[proc->personality]) {
		...
		sprintf(result, "SYS_%s<%d>",
			syscalents[proc->personality][sysnum], sysnum);
		return result;
	}
}
...
handle_breakpoint(Event *event) {
	...
	for (i = event->proc->callstack_depth - 1; i >= 0; i--) {
		if (event->e_un.brk_addr ==
			...
			assert(sym);
			void *addr =  sym2addr(event->proc,sym);
			//struct Breakpoint *tbp = dict_find_entry(event->proc->breakpoints, sym2addr(event->proc,sym));
			struct Breakpoint *tbp = dict_find_entry(event->proc->breakpoints, addr);
			// 2010/11/09, htf
			// address in .Got entry will be changed from stub to shared lib
			if ( tbp == 0 )
			{
			    // Add a  new dict for .got entry that was pointed to function's entry in shared lib
			    insert_breakpoint(event->proc, addr, sym);
			    tbp = dict_find_entry(event->proc->breakpoints, addr);
			}
			old_addr = tbp->addr;
...
callstack_push_symfunc(Process *proc, struct library_symbol *sym) {
	...
	debug(DEBUG_FUNCTION, "callstack_push_symfunc(pid=%d, symbol=%s) sym:%p", proc->pid, sym->name, sym);
	/* FIXME: not good -- should use dynamic allocation. 19990703 mortene. */
	if (proc->callstack_depth == MAX_CALLDEPTH - 1) {
		fprintf(stderr, "Error: call nesting too deep!\n");
		return;
