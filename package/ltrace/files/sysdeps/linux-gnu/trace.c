...
#if defined __sparc__  || defined __ia64___ || __mips__
// htf, should mips use this ???
...
continue_after_breakpoint(Process *proc, Breakpoint *sbp) {
	...
	if (sbp->enabled == 0) {
		...
		debug(DEBUG_PROCESS, "continue_after_breakpoint: pid=%d, addr=%p, breakpoint_being_enabled = %p", proc->pid, sbp->addr, sbp);
