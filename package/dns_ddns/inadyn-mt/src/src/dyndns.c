...
#include <openssl/sha.h> //Added by Fallen for FreeDNS
...
static char* get_freedns_update_hash(DYN_DNS_CLIENT *p_self, int cnt, char rsp_buffer[]); //Added by Fallen
...
DYNDNS_SYSTEM_INFO dns_system_table[] =
    {
		...
		{TZO_DEFAULT,
			...
			DYNDNS_MY_IP_SERVER, DYNDNS_MY_IP_SERVER_URL,
			"rh.tzo.com", "/webclient/tzoperl.html?tzoname=", NULL}}, //Modified by Fallen, tzo uses new protocol
...
{
	//Added by Fallen
	char rsp_buffer[DYNDNS_HTTP_RESPONSE_BUFFER_SIZE] = "";
	char *hash_ptr;
	if(DEBUG_BY_FALLEN)
	DBG_PRINTF((LOG_WARNING, "p_self2 = %p\n", p_self));
	hash_ptr = get_freedns_update_hash(p_self, cnt, rsp_buffer);
	if(hash_ptr == 0)
	return sprintf(p_self->p_req_buffer, FREEDNS_UPDATE_MY_IP_REQUEST_FORMAT,
	               "??",
	               p_self->info.dyndns_server_name.name[ip_store]);
	//End of code that added by Fallen
	(void)p_sys_info;
	return sprintf(p_self->p_req_buffer, FREEDNS_UPDATE_MY_IP_REQUEST_FORMAT,
	               hash_ptr,
	               p_self->info.dyndns_server_name.name[ip_store]);
...
//Added by Fallen to handle FreeDNS update hash request
static char* get_freedns_update_hash(DYN_DNS_CLIENT *p_self, int cnt, char rsp_buffer[])
{
	if(DEBUG_BY_FALLEN)
			...
			if (!(p_self->alias_info.update_required[i])) {
				...
				if (!((rc=http_client_init(&p_self->http_to_dyndns))==RC_OK)) {
					...
					/*build dyndns transaction*/
					{
						...
						if(DEBUG_BY_FALLEN)
						printf("FreeDNS, the second recieved data: [%s]\n", p_self->p_work_buffer);
