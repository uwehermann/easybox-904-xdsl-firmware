...
#endif /*ARCADYAN_VER*/
{
  ...
  #ifdef ARCADYAN_VER
	if (sVirtUsrName)
		ret = pdb_getsampwnam(sampass, sVirtUsrName);
	else
...
static NTSTATUS check_sam_security_arcadyan(
				...
				)
{
	...
	if ( (search = pdb_search_users(ACB_NORMAL)) == NULL )
		return NT_STATUS_NO_SUCH_USER;

	memset( &entry, 0, sizeof(entry) );
DEBUG(4,("check_sam_security_arcadyan: check user %s\n",user_info->internal_username));
	...
	while (1) {
		...
		nt_status = check_sam_security(auth_context, my_private_data, mem_ctx, user_info, server_info, entry.account_name);
DEBUG(4,("check_sam_security_arcadyan 2: check user %s ret %d\n",entry.account_name,nt_status));
		if (NT_STATUS_IS_OK(nt_status))
			break;
	}
...
static NTSTATUS auth_init_sam_ignoredomain(struct auth_context *auth_context, const char *param, auth_methods **auth_method) 
{
  ...
  #ifdef ARCADYAN_VER
	(*auth_method)->auth = check_sam_security_arcadyan;	
  #else
					 ...
					 auth_serversupplied_info **server_info)
{
  ...
  #ifdef ARCADYAN_VER
	return check_sam_security_arcadyan(auth_context, my_private_data, mem_ctx, user_info, server_info);
  #else
