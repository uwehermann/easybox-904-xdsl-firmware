...
int check_wlan(char *macaddr)
{
	FILE *fp;
	char line[100];
	char list[6][20];

	/* Parse wireless assoclist file */
	if ((fp = fopen("/tmp/wl_assoclist", "r"))) 
	{    
		while( fgets(line, sizeof(line), fp) != NULL ) 
		{    
			memset(list, 0, sizeof(list));
			sscanf(line,"%s %s %s %s %s %s",list[0], list[1], list[2], list[3], list[4], list[5]); //assoclist MAC wl_mode tx_rate rx_rate rssi

			if(strcmp(list[0],"assoclist"))
			{    
				continue;
			}    
			if (strcasecmp(macaddr, list[1]) == 0)
			{   
				fclose(fp);
				return 1;
			}
...
{
	char tmpbuf[1024];
	snprintf(tmpbuf, sizeof(tmpbuf), "received DISCOVER from %s", 
			(packet -> raw -> htype
		  ? print_hw_addr (packet -> raw -> htype,
				   packet -> raw -> hlen,
				   packet -> raw -> chaddr)
		  : (lease
		     ? print_hex_1(lease->uid_len, lease->uid, 60)
		     : "<no identifier>")));
	append_to_file(DHCP_LOG_TMP, tmpbuf);
}
#endif
...
{
	char tmpbuf[1024];
	snprintf(tmpbuf, sizeof(tmpbuf), "received REQUEST from %s", 
			(packet -> raw -> htype
		  ? print_hw_addr (packet -> raw -> htype,
				   packet -> raw -> hlen,
				   packet -> raw -> chaddr)
	...
	if (lease && lease -> pool && lease -> pool -> failover_peer) {
		...
		if (peer -> service_state == not_responding ||
			...
			log_info ("%s: not responding%s",
				  msgbuf, peer -> nrr);
			sprintf(cmd_str, "umng_syslog_cli addErrorCode H003");
			osSystem(cmd_str);
			goto out;
		}
...
{
	char tmpbuf[1024];
	snprintf(tmpbuf, sizeof(tmpbuf), "received RELEASE from %s", 
			(packet -> raw -> htype
		  ? print_hw_addr (packet -> raw -> htype,
				   packet -> raw -> hlen,
				   packet -> raw -> chaddr)
...
{
	char tmpbuf[1024];
	snprintf(tmpbuf, sizeof(tmpbuf), "received DECLINE from %s", 
			(packet -> raw -> htype
		  ? print_hw_addr (packet -> raw -> htype,
				   packet -> raw -> hlen,
				   packet -> raw -> chaddr)
...
{
	char tmpbuf[1024];
	snprintf(tmpbuf, sizeof(tmpbuf), "sending NAK to %s", 
			print_hw_addr (packet -> raw -> htype,
			packet -> raw -> hlen,
			packet -> raw -> chaddr));
	append_to_file(DHCP_LOG_TMP, tmpbuf);
...
#ifdef HAVE_DHCPD_LOGFILE
{
	...
	snprintf(tmpbuf, sizeof(tmpbuf), "sending %s %s", 
			(offer==DHCPOFFER)?"OFFER to 255.255.255.255 with":"ACK to", piaddr(lease->ip_addr));
	...
	struct lease *lease;
{
	...
	if(state -> offer == DHCPOFFER)
	{
		...
		memset(get_ipaddr, 0, sizeof(get_ipaddr));
		memset(macaddr, 0, sizeof(macaddr));
		sprintf(cmd_str, "ccfg_cli get dhcp4lease@lan0");
		osSystem_GetOutput( cmd_str, get_lease, sizeof(get_lease) );
		sprintf(cmd_str, "ccfg_cli get ip4addr@lan0");
		osSystem_GetOutput( cmd_str, get_ipaddr, sizeof(get_ipaddr) );

		time(&t);
		loctime = localtime(&t);
		strftime(date_time, sizeof(date_time), "%d.%m.%Y %H:%M:%S", loctime);
		strcpy(macaddr,print_hw_addr (lease -> hardware_addr.hbuf [0],lease -> hardware_addr.hlen - 1,&lease -> hardware_addr.hbuf [1]) );
		is_wlan=check_wlan(&macaddr);

		sprintf(cmd_str, "umng_syslog_cli addEventCode -10 H001 %s %s %s %s %s %s %s %s %s %s",
				date_time, //date
				"", //time
				is_wlan? "WLAN" : "LAN", //network LAN or WLAN
				s ? s : "", //computer neme
				macaddr, //MAC address
				piaddr (lease -> ip_addr),//IP address
				print_dotted_quads (lease -> subnet -> netmask.len, lease -> subnet -> netmask.iabuf),//subnet mask
				get_ipaddr, //DNS-server
				get_ipaddr, //gateway
				get_lease //lease time
				);
		osSystem(cmd_str);
	}
...
int arpping(struct lease *candl, u_int32_t ip, unsigned char *mac, char *interface)
{
	...
	if (setsockopt(s, SOL_SOCKET, SO_BROADCAST, &optval, sizeof(optval)) == -1) {
		log_error("Could not setsocketopt on raw socket");
		close(s);
		return -1;
	}
	...
	memcpy(arp.ethhdr.ether_dhost, "\xff\xff\xff\xff\xff\xff", 6);	/* MAC DA */
...
#if defined (DEBUG_FIND_LEASE)
	log_info ("%salid arp replies for this address (%s).", rv ? "V" : "No v", piaddr(candl->ip_addr));
#endif
	return rv;
}
