...
// 2011-Feb-23 ruby change
static int server_duid_type = /*DUID_LLT*/ DUID_LL;
 ...
 */
isc_result_t
//generate_new_server_duid(void) { /*Clyde*/
generate_new_server_duid(u_int32_t dhcp6s_uptime) {
	...
	if (server_duid_type == DUID_LLT) {
		...
		putUShort(generated_duid.buffer->data + 2,
			  p->hw_address.hbuf[0]);
//		putULong(generated_duid.buffer->data + 4, time_val); /*Clyde*/
		putULong(generated_duid.buffer->data + 4, dhcp6s_uptime);
		...
		const struct data_string *server_id)
{
	...
	/* hsubj add to set UseMulticast status code when received a dhcpv6 request, renew, release or declient 
           message with unicast destination MAC address */
	if ((packet->unicast == ISC_TRUE) && 
	    ((reply.packet->dhcpv6_msg_type == DHCPV6_REQUEST) ||
	     (reply.packet->dhcpv6_msg_type == DHCPV6_RENEW) ||
	     (reply.packet->dhcpv6_msg_type == DHCPV6_RELEASE) ||
	     (reply.packet->dhcpv6_msg_type == DHCPV6_DECLINE))) {
		if (!set_status_code(STATUS_UseMulticast, "", reply.opt_state)) {
                        log_error("lease_to_client: Unable to set "
                                  "STATUS_UseMulticast status code.");
                        goto exit;
                }
...
skip_ia_pd:
	...
	time(&t);
	loctime = localtime(&t);
	strftime(date_time, sizeof(date_time), "%d.%m.%Y %H:%M:%S", loctime);
	sprintf(cmd_str, "umng_syslog_cli addEventCode -3 DH101 %s %s %s",
			date_time, //date
			"", //time
			"fe80::1" //DNS-server
			);
...
reply_process_ia_pd(struct reply_state *reply, struct option_cache *ia) {
	...
	if ((status != ISC_R_CANCELED) && (reply->static_prefixes == 0) &&
		...
		for (i = 0 ; i < reply->ia->num_iasubopt ; i++) {
			...
			inet_ntop(AF_INET6, &tmp->addr, addr_buf, sizeof(addr_buf));
			sprintf(cmd, "/sbin/route -A inet6 add %s/%d gw %02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x", addr_buf, (int)tmp->plen, reply->packet->client_addr.iabuf[0], reply->packet->client_addr.iabuf[1],reply->packet->client_addr.iabuf[2],reply->packet->client_addr.iabuf[3],reply->packet->client_addr.iabuf[4],reply->packet->client_addr.iabuf[5],reply->packet->client_addr.iabuf[6],reply->packet->client_addr.iabuf[7],reply->packet->client_addr.iabuf[8],reply->packet->client_addr.iabuf[9],reply->packet->client_addr.iabuf[10],reply->packet->client_addr.iabuf[11],reply->packet->client_addr.iabuf[12],reply->packet->client_addr.iabuf[13],reply->packet->client_addr.iabuf[14],reply->packet->client_addr.iabuf[15]);
			system(cmd);
		}
		   ...
		   void (*ia_na_nomatch)())
{
	...
	/* hsubj add to set UseMulticast status code when received a dhcpv6 
	 * request, renew, release or declient message with unicast destination MAC address */
	if ((packet->unicast == ISC_TRUE) &&
	    ((packet->dhcpv6_msg_type == DHCPV6_REQUEST) ||
	    (packet->dhcpv6_msg_type == DHCPV6_RENEW) ||
	    (packet->dhcpv6_msg_type == DHCPV6_RELEASE) ||
	    (packet->dhcpv6_msg_type == DHCPV6_DECLINE))) {
		if (!set_status_code(STATUS_UseMulticast, "", opt_state)) {
			log_error("iterate_over_ia_na: Unable to set "
				"STATUS_UseMulticast status code.");
			goto exit;
		}
		wrong_msg = ISC_TRUE;
