	...
	struct lease **lpchain;
{
	...
	for (i = 0; i < max - min + 1; i++) {
		...
		if (find_lease_by_ip_addr (&lt, lp -> ip_addr, MDL)) {
			if (lt -> pool) {
				// In order to write dhcpd-br0.conf conveniently.
//				parse_warn (cfile,
...
//#define LEASE_FILE				"/tmp/udhcpd.leases"
//#define DELETE_LEASE_FILE		"/tmp/.delete_leases"
//dump lease info to file.	2010-11-11
extern const char *path_dhcpd_lease;
extern const char *path_dhcpd_deleted;
int dump_leases()
{
...
#endif
			{
                        	lease_time = htonl(l->ends - cur_time);
                        	fwrite(&(l -> hardware_addr.hbuf[1]), 16, 1, fp);
                        	fwrite(&(l -> ip_addr), sizeof(struct iaddr), 1, fp);
                        	fwrite(&lease_time, 4, 1, fp);
				if(l ->client_hostname)
					fwrite(l ->client_hostname, 64, 1, fp);
				else
					fwrite("", 64, 1, fp);
			    	nwritten++;
			}
...
int delete_lease()
{
	...
	i = 0;
	if ((fp = fopen(path_dhcpd_deleted, "r"))!=NULL) 
	{
...
//tlhhh 2010-1-24. random foreach to enqueue the leases in lease_ip hash
int lease_ip_hash_random_foreach (table, func)
...
{   
    ...
    for (i = 0; i < table->hash_count; i++) {
        ...
        while (bp) {
			...
			//t = (struct lease *)bp->value;
			//log_info("%d: i=%d, bp->value=%s", __LINE__, i, piaddr(t->ip_addr));

			if ( k >= RANDOM_POOL_SIZE ) {
				// exceed the number we can buffer
				if ((*func)(bp->name, bp->len, bp->value) != ISC_R_SUCCESS)
					return count;
			}
			else {
				place[k++] = i;
...
loop:
	//log_info("%d: min=%d, max=%d, tbl->cnt=%d", __LINE__, min, max, table->hash_count);
	for (i=min; i<max; i++ ) {
		bp = table -> buckets [place[i]];
		while (bp) {
			next = bp -> next;
