...
const char *path_dhcpd_lease = _LEASE_FILE;
const char *path_dhcpd_deleted = _DELETE_LEASE_FILE;
/* 2011-Feb-01 Jerry add */
const char *path_dhcpd6_lease = _LEASE6_FILE;
const char *path_dhcpd6_deleted = _DELETE_LEASE6_FILE;
		...
		if (!strcmp (argv [i], "-p")) {
			...
			for (s = argv [i]; *s; s++)
			    if (!isdigit ((unsigned char)*s))
			        log_fatal ("%s: not a valid UDP port", argv [i]);
			status = atoi(argv [i]);
			...
			no_dhcpd_pid = 1;
		} else if (!strcmp (argv [i], "-df")) {	//Alpha add to accept display lease file
			if (++i == argc)
				usage ();
			path_dhcpd_lease = argv [i];
			no_dhcpd_lease = 1;
...
#  endif /* EARLY_CHROOT */
	...
	//handle signals	Add by Alpha 2010-11-11
	...
	if (set_server_duid_from_option() == ISC_R_SUCCESS) {
		...
		if (!server_duid_isset()) {
			if (generate_new_server_duid(dhcp6s_uptime) != ISC_R_SUCCESS) {
	...
	if ((i = open (path_dhcpd_pid, O_RDONLY)) >= 0) {
		...
		if (status > 0) {
			...
			{
				char buff[50];
				printf("There's already a DHCP server running. kill %d\n", pid);
