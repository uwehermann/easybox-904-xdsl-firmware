...
create_prefix6(struct ipv6_pool *pool, struct iasubopt **pref, 
	...
	*attempts = 0;
	for (;;) {
		...
		/* hsubj add for static DHCPv6 mapping function */
		memset(&tmp, 0, sizeof(tmp));
		if (uid->data != NULL) {
			unsigned char *peer_hw_addr = NULL;
			unsigned short duid_type = getUShort(uid->data);
			unsigned short hardware_type = getUShort(uid->data+2);
			struct host_decl *host = NULL;
			if (hardware_type == 0x06) {
				if (duid_type == DUID_LLT) {
					peer_hw_addr = (unsigned char *)uid->data + 8;
				}
				else if (duid_type == DUID_LL) {
					peer_hw_addr = (unsigned char *)uid->data + 4;
				}
				if (peer_hw_addr != NULL) {
					if (find_hosts_by_haddr(&host, HTYPE_ETHER, peer_hw_addr, 6, MDL)) {
						if ((host != NULL)  && (host->fixed_prefix != NULL)) {
							memcpy(&tmp, host->fixed_prefix->cidrnet.lo_addr.iabuf, 16);
						}
                                        }
				}
			}
...
extern const char *path_dhcpd6_lease;
extern const char *path_dhcpd6_deleted;

static isc_result_t 
dump_ia_leases(const void *name, unsigned len, void *value) {
	struct ia_xx *ia = (struct ia_xx *)value;
    int i;
	...
	u_int32_t addr_len;
	unsigned char null_mac[]={0x00, 0x00, 0x00, 0x00, 0x00, 0x00};


	/* Get the current time... */
	gettimeofday(&cur_tv, NULL);
	
    client_duid_len = ia->iaid_duid.len - 4 ;
	htype = getUShort(ia->iaid_duid.data + 4 + 2);
	hlen = 0;
	chaddr = NULL;
	lease_time = 0;
	if ((fp = fopen(path_dhcpd6_lease, "a")) == NULL)
	{
		return -1;
	}
	switch(getUShort(ia->iaid_duid.data+ 4 )) {
	      case DUID_LLT:
		if (client_duid_len > 8) {
			hlen = client_duid_len - 8;
			chaddr = ia->iaid_duid.data + 4 + 8;
		}
		break;
		 ...
		 */
		hlen = client_duid_len - 4;
		chaddr = ia->iaid_duid.data + 4 + 4;
		break;
	...
	if(chaddr)
		fwrite(chaddr, 6, 1, fp);
	else
		fwrite(null_mac, 6, 1, fp);
	...
	for (i=0; i<ia->num_iasubopt; i++) {
		...
		fwrite(&ia->ia_type, 2, 1, fp);
		fwrite(&iasubopt->addr, sizeof(struct in6_addr), 1, fp);
		addr_len = (u_int32_t)iasubopt->plen;
		fwrite(&addr_len, 4, 1, fp);	
		...
		if ((iasubopt->state == FTS_ACTIVE) ||
		    ...
		    (iasubopt->hard_lifetime_end_time != 0)) {
				lease_time = iasubopt->hard_lifetime_end_time - cur_time;
		} else {
			lease_time = iasubopt->soft_lifetime_end_time - cur_time;
		}
...
dump_leases6(void) {
	...
	if ((fp = fopen(path_dhcpd6_lease, "w")) == NULL)
	{
		return 0;
	}
	fclose(fp);
	ia_hash_foreach(ia_na_active, dump_ia_leases);

	return 1;
}
static isc_result_t 
delete_ia_leases6(const void *name, unsigned len, void *value) {
	struct ia_xx *ia = (struct ia_xx *)value;
	FILE *fp;
	int count, i, j;
	char line[80];
	...
	/* read "/tmp/.delete_leases6" which generated from DHCPTable.asp */

	i = 0;
	if ((fp = fopen(path_dhcpd6_deleted, "r"))!=NULL) 
	{
		while(fgets(line,sizeof(line),fp)!=NULL)
		{
			strncpy(table[i], line, strlen(line)-1);
			i++;
		}
	   fclose(fp);
	}
...
int delete_lease6(void)
{
	ia_hash_foreach(ia_na_active, delete_ia_leases6);
	return 1;
}
