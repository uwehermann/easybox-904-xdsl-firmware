...
/* ruby add for detecting IPv6 initiation failure stage */
//#define TX_COUNT_MAX 3

//2011-03-29 ruby add to fallback to solicit when DUT do rebind failed after hot plug wan port.
extern int fallback2solicit;
...
unsigned long dhcp6c_prefix_mode = 0; // 0: random-prefix mode; 1: preferred-prefix mode
#endif
/*
	...
	for ( ; oc != NULL ; oc = oc->next) {
		...
		if (pfx == NULL) {
			...
			if ((pfx->preferred_life > pfx->max_life) ||
				((pfx->preferred_life == 0) && (pfx->max_life == 0))/*rfc 3315 section 18.2.4 reply 0 life time to indicate invalid */) {
				log_debug("RCV:  | | | !-- INVALID lifetimes, "
...
start_init6(struct client_state *client)
{
	...
	iprintf("[start_init6] PRC: Soliciting for leases (INIT).\n");
...
start_confirm6(struct client_state *client)
{
	...
	if ((client->active_lease == NULL) ||
		...
		#ifdef DHC6_DEBUG
		iprintf("call start_init6(client) , line:%d\n",__LINE__) ;
		#endif	// DHC6_DEBUG
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c start_confirm6] call do_refresh6 , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
do_init6(void *input)
{
	...
	struct dhc6_ia *old_ia=NULL;
	struct dhc6_addr *old_addr=NULL;
	struct data_string ds;
	...
	if (client->advertised_leases != NULL) {
		iprintf("[do_init6] client->advertised_leases != NULL, call start_selecting6()\n");
	...
	if ((client->MRC != 0) && (client->txcount > client->MRC)) {
		...
		osSystem("umng_syslog_cli addErrorCode P004");
		osSystem("umng_syslog_cli addErrorCode -1 P006 \"no server response.\"");
		/* Need send signal to pppd. */
		osSystem("killall -s USR1 pppd");
		exit(2);
	}
	...
	for (i = 0; i < wanted_ia_na; i++) {
...
#if 0 /* 2011-Feb-10 bruce add to cd-router (IPV6) */
		if ((client->active_lease != NULL) &&
...
#if 1 /* 2011-Feb-10 bruce add to cd-router (IPV6) */
		if ((client->active_lease != NULL) && (old_ia != NULL))
		{
#endif
		...
		else if(prefered_ia_na.address.len == 16)
		{
				if (!buffer_allocate(&addr.buffer, 24, MDL)) {
					log_error("Unable to allocate memory "
						  "for IAADDR.");
				...
				log_debug("XMT:  | B-- Request address %s.",
					  piaddr(prefered_ia_na.address));
				log_debug("XMT:  | | B-- Request "
					  "preferred in +%u",
					  (unsigned)t1);
				log_debug("XMT:  | | B-- Request valid "
					  "in     +%u",
					  (unsigned)t2);
	...
	for (i = 0; i < wanted_ia_pd; i++) {
		...
		iprintf("[do_init6] XMT:  X-- IA_PD %s\n",
			  print_hex_1(4, ia.buffer->data, 55));
		iprintf("[do_init6] XMT:  | X-- Request renew in  +%u\n", (unsigned)t1);
		iprintf("[do_init6] XMT:  | X-- Request rebind in +%u\n", (unsigned)t2);

#if 0 /* 2011-Feb-10 bruce add for cd-router (IPV6) */
		if ((client->active_lease != NULL) &&
		    ((old_ia = find_ia(client->active_lease->bindings,
				       D6O_IA_PD,
				       (char *)ia.buffer->data)) != NULL)) {
#else
		iprintf("[do_init6] prefered_ia_pd.address.len=%d, line %d\n", prefered_ia_pd.address.len, __LINE__);
		if (((client->active_lease != NULL) &&
		    ((old_ia = find_ia(client->active_lease->bindings,
				       D6O_IA_PD,
				       (char *)ia.buffer->data)) != NULL))
		     || (prefered_ia_pd.address.len > 0)) {
#endif
			/*
		...
		else if (prefered_ia_pd.address.len == 16)
		{
			iprintf("[do_init6] copy preferred_ia_pd, line %d\n", __LINE__);
				if (!buffer_allocate(&addr.buffer, 25, MDL)) {
					log_error("Unable to allocate memory "
						  "for IAPREFIX.");
					data_string_forget(&ia, MDL);
				...
				putUChar(addr.buffer->data + 8,
					 prefered_ia_pd.plen);
				memcpy(addr.buffer->data + 9,
				       prefered_ia_pd.address.iabuf,
				       16);

				log_debug("XMT:  | B-- Request prefix %s/%u.",
					  piaddr(prefered_ia_pd.address),
					  (unsigned) prefered_ia_pd.plen);
				log_debug("XMT:  | | B-- Request "
					  "preferred in +%u",
					  (unsigned)t1);
				log_debug("XMT:  | | B-- Request valid "
				...
				iprintf("[do_init6] XMT:  | B-- Request prefix %s/%u.\n",
					  piaddr(prefered_ia_pd.address),
					  (unsigned) prefered_ia_pd.plen);
				iprintf("[do_init6] XMT:  | | B-- Request "
					  "preferred in +%u\n",
					  (unsigned)t1);
				iprintf("[do_init6] XMT:  | | B-- Request valid "
					  "in     +%u\n",
					  (unsigned)t2);
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_init6] send DHCPv6 Solicit , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
#if 1	/*initial waiting times followed alg_01 of TOCPE0051*/
	/* Wait RT */
	if(client->txcount)
		t1 = 1 << (client->txcount - 1);
	else
		t1 = 1;
	tv.tv_sec = cur_tv.tv_sec + t1;
	tv.tv_usec = cur_tv.tv_usec;
	log_debug("dhc6: cur_tv.tv_sec[%d], t1[%d], client->txcount[%d]",
			  cur_tv.tv_sec, t1, client->txcount);
#else
	/* Wait RT */
...
do_info_request6(void *input)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_info_request6] call send_packet6 , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
do_confirm6(void *input)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_confirm6] call start_bound() , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_confirm6] call send_packet6 , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
do_release6(void *input)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_release6] call start_bound() , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
	...
	memcpy(ds.buffer->data + 1, client->dhcpv6_transaction_id, 3);
        ...
        if (client->elapsed == 0)
                log_debug("XMT: Forming Release, 0 ms elapsed.");
        else
                log_debug("XMT: Forming Release, %u0 ms elapsed.",
                          (unsigned)client->elapsed);
...
start_decline6(struct client_state *client)
{
        ...
        /*
         * It is written:  "The client MUST NOT use any of the addresses it
         * is declining as the source address in the Decline message or in
         * any subsequently transmitted message."  So unconfigure now.
         */
        unconfigure6(client, "DECLINE6");
...
do_decline6(void *input)
{
        ...
        struct data_string ds;
        int send_ret;
        struct timeval elapsed, tv; /* grace add elapsed option */
	...
	if (client->elapsed == 0)
		log_debug("XMT: Forming Decline, 0 ms elapsed.");
	else
		log_debug("XMT: Forming Decline, %u0 ms elapsed.",
			  (unsigned)client->elapsed);
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_decline6] call send_packet6 , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
dhc6_check_advertise(struct dhc6_lease *lease)
{
	...
	struct dhc6_addr *addr;
	isc_result_t rval = ISC_R_SUCCESS;
	isc_result_t saved_rval; /* 2010-12-07 bruce add */
	...
	iprintf("[dhc6.c dhc6_check_advertise] enter\n");
	...
	for (ia = lease->bindings ; ia != NULL ; ia = ia->next) {
		...
		/* 2010-12-07 bruce add to support IA_NA */
		if((ia->ia_type == D6O_IA_NA) && (rval != ISC_R_SUCCESS))
		{
			isc_result_t status;
			dhc6_ia_destroy(ia, MDL);
			status = dhc6_get_status_code(ia->options, &code, NULL);
			printf("%d %d\n",status, code);
			if(((status != ISC_R_SUCCESS) || (code != STATUS_Success)) && (saved_rval == ISC_R_SUCCESS))
			rval = ISC_R_SUCCESS;
			wanted_ia_na = 0;
			continue;
		}
...
/*				if(addr->plen != 56)
	...
	/*
	 * Check whether the server has sent valid DNS infomation.
	 */
	iprintf("[dhc6_check_advertise] Check whether the server has sent valid DNS infomation.\n");

	log_debug("Check whether the server has sent valid DNS infomation.");
	memset(&ds, 0, sizeof(ds));
	oc = lookup_option(&dhcpv6_universe, lease->options, D6O_NAME_SERVERS);
					...
					oc, MDL))
	{
		if(ds.len > 0)
		{
			for(i=0; i<ds.len/16; i++)
				iprintf("[dhc6_check_advertise] [%02x %02x %02x %02x %02x %02x %02x %02x  %02x %02x %02x %02x %02x %02x %02x %02x]\n"
					,ds.data[i*16],ds.data[i*16+1],ds.data[i*16+2],ds.data[i*16+3],ds.data[i*16+4],ds.data[i*16+5],ds.data[i*16+6],ds.data[i*16+7]
					,ds.data[i*16+8],ds.data[i*16+9],ds.data[i*16+10],ds.data[i*16+11],ds.data[i*16+12],ds.data[i*16+13],ds.data[i*16+14],ds.data[i*16+15]);
	...
	{
		iprintf("[dhc6_check_advertise] The server has not sent valid DNS infomation.\n");

		log_error("The server has not sent valid DNS infomation.");
		rval = ISC_R_SUCCESS;// yuzhang modified , ignore the fail, becuse maybe get DNS from RA
		//rval = ISC_R_FAILURE;
	}
	data_string_forget(&ds, MDL);
...
init_handler(struct packet *packet, struct client_state *client)
{
	...
	iprintf("[dhc6.c init_handler] enter\n");
	...
	if ((client->txcount > 1) ||
		...
		iprintf("[init_handler] RCV:  Advertisement immediately selected.\n");
		cancel_timeout(do_init6, client);
		start_selecting6(client);
	} else
	{
		iprintf("[init_handler] RCV:  Advertisement recorded.\n");
...
rapid_commit_handler(struct packet *packet, struct client_state *client)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c rapid_commit_handler] call start_bound() , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
start_selecting6(struct client_state *client)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c start_selecting6] call do_select6(client) , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
do_select6(void *input)
{
	...
	if (wanted_ia_na &&
		...
		data_string_forget(&ds, MDL);
		return; /* 2010-12-07 bruce add to support IA_NA */
	}
	if (wanted_ia_ta &&
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_select6] send DHCPv6 Request , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
reply_handler(struct packet *packet, struct client_state *client)
{
	...
	if (client->state == S_REBOOTING) {
		...
		#ifdef DHC6_DEBUG
		iprintf("[dhc6.c reply_handler] call start_bound() , line:%d\n",__LINE__) ;
		#endif	// DHC6_DEBUG
		     ...
		     struct dhc6_addr *addr)
{
	...
	if (addr != NULL) {
		if ((ia != NULL) && (ia->ia_type == D6O_IA_PD)) {
			 ...
			 */
			// 2011-0517 modified 64->128
			// (UNH Plugfest: if L-flag=0, router should not add any rule to routing table.
			// All traffic should go to default gateway.)
		...
		client_envadd(client, prefix, "life_starts", "%d",
			      (int)(addr->starts));
		client_envadd(client, prefix, "preferred_life", "%lld",
			      (long long)(addr->preferred_life));
		client_envadd(client, prefix, "max_life", "%lld",
			      (long long)(addr->max_life));
	}
...
dhc6_check_times(struct client_state *client)
{
	...
	#ifdef DHCPv6
	iprintf("[dhc6.c dhc6_check_times] enter , line:%d\n",__LINE__) ;
	#endif	// DHCPv6
	...
	switch(client->state) {
		...
		#ifdef DHCPv6
		iprintf("[dhc6.c dhc6_check_times] cur_time=%lu , renew=%lu , rebind=%lu , line:%d\n",cur_time,renew,rebind,__LINE__) ;
		#endif	// DHCPv6
		if ((rebind > cur_time) && (renew < rebind)) {
			...
			#ifdef DHCPv6
			iprintf("[dhc6.c dhc6_check_times] call start_renew6 , line:%d\n",__LINE__) ;
			#endif	// DHCPv6
	...
	for (sia = src->bindings ; sia != NULL ; sia = sia->next) {
		...
		if (dia == NULL) {
			...
			for (saddr = sia->addrs ; saddr != NULL ;
				...
				if (daddr == NULL) {
					#if 0 /*rfc 3315 section 18.2.4 reply 0 life time to indicate invalid */
...
start_bound(struct client_state *client)
{
	...
	/* hsubj add to trigger new ip settings available */
	int trigger_effect = 0;
	// bitonic
	#if 1
	int ipv6_tid = MID_FAIL;
	#endif

	iprintf("[dhc6.c start_bound] enter , line:%d\n",__LINE__);
	...
	switch (client->state) {
		...
		reason = "BOUND6";
		iprintf("[start_bound] reason=%s, line %d\n", reason, __LINE__);
		break;
		...
		break;
	      ...
	      default:
		log_fatal("Impossible condition at %s:%d.", MDL);
		iprintf("[start_bound] Impossible condition at %s:%d., line %d\n", MDL, __LINE__);
	...
	iprintf("[start_bound] PRC: Bound to lease %s.\n",
		  print_hex_1(client->active_lease->server_id.len,
			      client->active_lease->server_id.data, 55));
...
#if 1 /* 2011-Feb-10 bruce add for cd-router (IPV6) */
		if(ia->ia_type == D6O_IA_NA){
			prefered_ia_na.address = ia->addrs->address;
			iprintf("[start_bound] prefered ia_na = %d %x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x\n", prefered_ia_na.address.len,
				prefered_ia_na.address.iabuf[0], prefered_ia_na.address.iabuf[1], prefered_ia_na.address.iabuf[2], prefered_ia_na.address.iabuf[3],
				prefered_ia_na.address.iabuf[4], prefered_ia_na.address.iabuf[5], prefered_ia_na.address.iabuf[6], prefered_ia_na.address.iabuf[7],
				prefered_ia_na.address.iabuf[8], prefered_ia_na.address.iabuf[9], prefered_ia_na.address.iabuf[10], prefered_ia_na.address.iabuf[11],
				prefered_ia_na.address.iabuf[12], prefered_ia_na.address.iabuf[13], prefered_ia_na.address.iabuf[14], prefered_ia_na.address.iabuf[15]);
		}

		if(ia->ia_type == D6O_IA_PD){
			#if 1
			// bitonic
			if(dhcp6c_prefix_mode == 0)
			{	// randiom-prefix mode
				prefered_ia_pd.address = ia->addrs->address;
				prefered_ia_pd.plen = ia->addrs->plen;

				iprintf("[start_bound] prefered ia_pd = %d %x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x\n", prefered_ia_pd.address.len,
					prefered_ia_pd.address.iabuf[0], prefered_ia_pd.address.iabuf[1], prefered_ia_pd.address.iabuf[2], prefered_ia_pd.address.iabuf[3],
					prefered_ia_pd.address.iabuf[4], prefered_ia_pd.address.iabuf[5], prefered_ia_pd.address.iabuf[6], prefered_ia_pd.address.iabuf[7],
					prefered_ia_pd.address.iabuf[8], prefered_ia_pd.address.iabuf[9], prefered_ia_pd.address.iabuf[10], prefered_ia_pd.address.iabuf[11],
					prefered_ia_pd.address.iabuf[12], prefered_ia_pd.address.iabuf[13], prefered_ia_pd.address.iabuf[14], prefered_ia_pd.address.iabuf[15]);
			}
			#endif
		}
#endif
	...
	/* hsubj add to trigger new ip settings available */
	if (trigger_effect == 1) {
		script_init(client, "EFFECT6", NULL);
		script_go(client);
		trigger_effect = 0;
	}
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c start_bound] call dhc6_check_times() , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
start_renew6(void *input)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c start_renew6] enter , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c start_renew6] call do_refresh6 , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
do_refresh6(void *input)
{
	...
	if (((client->MRC != 0) && (client->txcount > client->MRC)) ||
		...
		#ifdef DHC6_DEBUG
		iprintf("[dhc6.c do_refresh6] call dhc6_check_times() , line:%d\n",__LINE__) ;
		#endif	// DHC6_DEBUG
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_refresh6] send dhcpv6 release , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG

	if((ipv6_tid=mapi_start_transc()) == MID_FAIL)
	{
		iprintf( "[dhc6.c do_refresh6] Failed to start a transaction to arc middle , line:%d\n",__LINE__);
		//return 0;
	}
	else
	{
		if( mapi_ccfg_get_long( ipv6_tid , "system", "ip6_dhcp6c_random_case", 0 ) == ON_DHCP_RENEW )
		{
			#ifdef DHC6_DEBUG
			iprintf("[dhc6.c do_refresh6] send dhcpv6 release , line:%d\n",__LINE__) ;
			#endif	// DHC6_DEBUG
			
			osSystem("/usr/sbin/dhcp6c_restart");
		}
	}
	...
	/* Wait RT */
	tv.tv_sec = cur_tv.tv_sec + client->RT / 100;
	/* hsubj add to send rebind message when time T2 is reached. (pass ipv6ready dhcpv6 rfc3315 item 48 */
	if ((client->refresh_type == DHCPV6_RENEW) && (tv.tv_sec > client->start_time.tv_sec + client->MRD)) {
		tv.tv_sec = client->start_time.tv_sec + client->MRD;
		client->RT = (client->start_time.tv_sec + client->MRD - cur_tv.tv_sec) * 100;
	}
	tv.tv_usec = cur_tv.tv_usec + (client->RT % 100) * 10000;
	if (tv.tv_usec >= 1000000) {
		tv.tv_sec += 1;
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_refresh6] call do_refresh6 , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
start_rebind6(void *input)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c start_rebind6] call do_refresh6 , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
do_depref(void *input)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_depref] call dhc6_check_times() , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
...
do_expire(void *input)
{
	...
	#ifdef DHC6_DEBUG
	iprintf("[dhc6.c do_expire] call dhc6_check_times() , line:%d\n",__LINE__) ;
	#endif	// DHC6_DEBUG
