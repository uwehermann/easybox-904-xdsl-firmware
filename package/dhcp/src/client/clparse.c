...
isc_result_t read_client_conf ()
{
	...
	*/
	...
	top_level_config.reboot_timeout = 31; //10; // avoid the request retry to over 32s
	top_level_config.retry_interval = 10;
	top_level_config.backoff_cutoff = 10;
	top_level_config.initial_interval = 3;//4;
	...
	if (default_request_lease_time != -1)
		top_level_config.requested_lease = default_request_lease_time;
	else
...
void read_client_leases ()
{
	...
	iprintf("[read_client_leases] path_dhclient_db=%s\n", path_dhclient_db);
	...
	do {
		...
		switch (token) {
		      case DEFAULT_DUID:
iprintf("[read_client_leases] DEFAULT_DUID\n");
			parse_client_default_duid(cfile);
			break;

		      case LEASE:
iprintf("[read_client_leases] LEASE\n");
			parse_client_lease_statement(cfile, 0);
			break;

		      case LEASE6:
iprintf("[read_client_leases] LEASE6\n");
...
parse_client6_lease_statement(struct parse *cfile)
{
	...
	while (token != RBRACE) {
		...
		switch(token) {
		      case IA_NA:
iprintf("[parse_client6_lease_statement] IA_NA\n");
			*ia = parse_client6_ia_na_statement(cfile);
			...
			break;
		      ...
		      case IA_TA:
iprintf("[parse_client6_lease_statement] IA_TA\n");
			*ia = parse_client6_ia_ta_statement(cfile);
			...
			break;
		      ...
		      case IA_PD:
iprintf("[parse_client6_lease_statement] IA_PD\n");
			*ia = parse_client6_ia_pd_statement(cfile);
			...
			break;
		      ...
		      case INTERFACE:
iprintf("[parse_client6_lease_statement] INTERFACE\n");
			if (iface != NULL) {
			...
			break;
		      ...
		      case NAME:
iprintf("[parse_client6_lease_statement] NAME\n");
			has_name = 1;
			...
			break;
		      ...
		      case OPTION:
iprintf("[parse_client6_lease_statement] OPTION\n");
			if (parse_option_decl(&oc, cfile)) {
...
iprintf("[parse_client6_lease_statement] TOKEN_RELEASED, TOKEN_ABANDONED\n");
	...
	if (!has_ia) {
		...
		iprintf("Lease with no IA's discarded from lease db.\n");
