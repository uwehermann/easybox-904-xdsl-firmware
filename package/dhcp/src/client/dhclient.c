...
extern struct dhc6_prefered_addr prefered_ia_na;
extern struct dhc6_prefered_addr prefered_ia_pd;
extern unsigned long dhcp6c_prefix_mode; // 0: random-prefix mode; 1: preferred-prefix mode
#endif
//

#define DHCLIENT_DEBUG
...
/* Signal handler */
#define SEND_RELEASE_PACKET	0
#define SEND_REQUEST_PACKET	1
#define RESEND_CNT				3
static int old_state;
static int renew_requested_cnt = 0;
...
/* dump as IPv6 address format */
void dumpv6addr(unsigned char *tmp)
{
    unsigned char i;

    for(i=0; i<16; i++)
	{
		if((i%2==0)&&(i!=0)) iprintf(":");
      	iprintf("%02X",tmp[i]);
	}
}

void state_check(cpp)
	void *cpp;
{
	struct client_state *client = cpp;
	cancel_timeout(state_check, client);
	if(client->state == S_REQUESTED)	//still in S_REQUESTED, means didn't receive any ack packet.
	{
		/* Cancel any timeouts. */
	...
	void *cpp;
{
	...
	if(client->state != S_REQUESTED)
	{
		renew_requested_cnt = 0;
		return;
	}

	if(renew_requested_cnt >= RESEND_CNT)
	{
		tv.tv_sec = cur_time + 5;
		tv.tv_usec = 0;
		add_timeout(&tv, state_check, client, 0, 0);
		return;
	}
	...
	add_timeout(&tv, send_single_request, client, 0, 0);
}

void do_requested(cpp)
	void *cpp;
{
	struct client_state *client = cpp;
	...
	send_single_request (client);
}


static void dhcpclient_action(int action)
{
	...
	for (ip = interfaces ; ip ; ip = ip->next)
	{
		...
		if(bridge_mode == 1)
		{
			if(ip->name && bridge_rcvif && !strcmp(ip->name,bridge_rcvif))
				continue;
		}
		for (client = ip->client; client; client = client->next)
		{
			if (action == SEND_RELEASE_PACKET)
			{
				do_release(client);
				if(client -> active)		//request new lease
				{
					destroy_client_lease (client -> active);
					client -> active = (struct client_lease *)0;
				}
			}
			else if (action == SEND_REQUEST_PACKET)
			{
				if((!client -> active)
					||(client->state == S_STOPPED)
					||(client->state == S_REBOOTING)
					||(client->state == S_INIT) 			//S_INIT, S_SELECTING are states before get an lease.
				...
				{
					do_requested(client);
					/* Set up a timeout to check the state.*/
					//tv.tv_sec = cur_time + 5;
					//tv.tv_usec = 0;
					//add_timeout(&tv, state_check, client, 0, 0);
				}
...
static void signal_handler(int sig)
{
	...
	{
	        case SIGUSR1:	//renew
			dhcpclient_action(SEND_REQUEST_PACKET);
			break;
	        case SIGUSR2:	//release
			dhcpclient_action(SEND_RELEASE_PACKET);
			break;
	        case SIGTERM:
		 default:
...
main(int argc, char **argv) {
...
#endif /* DHCPv6 */
	char *s;
	int Nothing2Do = 1; /* 2011-04-01 If no client info, currently running client will exit. */
	// bitonic
	#if 1
	int ipv6_tid = MID_FAIL;
	char dhcp6c_preferred_prefix_text_addr[INET6_ADDRSTRLEN];
	#endif
	...
	{
		iprintf( "[dhclient] Failed to start a transaction to arc middle\n" );
		return 0;
	}

	dhcp6c_prefix_mode = mapi_ccfg_get_long(ipv6_tid, "system", "ip6_dhcp6c_prefix_mode", 0);
	iprintf("[dhclient] dhcp6c_prefix_mode=%d , line:%d\n", dhcp6c_prefix_mode,__LINE__);

	#if 1	// kevin_cheng , 20130116
	if( dhcp6c_prefix_mode == PREFERRED_MODE )
	#else
	if(dhcp6c_prefix_mode == 1)
	#endif
	{	// preferred-prefix mode
		memset(dhcp6c_preferred_prefix_text_addr, 0, 40);
		prefered_ia_pd.plen = mapi_ccfg_get_long(ipv6_tid, "system", "ip6_dhcp6c_prefix_length", 0);
		mapi_ccfg_get_str(ipv6_tid, "system", "ip6_dhcp6c_prefix_addr", dhcp6c_preferred_prefix_text_addr, sizeof(dhcp6c_preferred_prefix_text_addr), "");
		iprintf("[dhclient] dhcp6c_preferred_prefix_text_addr=%s\n", dhcp6c_preferred_prefix_text_addr);

		inet_pton(AF_INET6, dhcp6c_preferred_prefix_text_addr, prefered_ia_pd.address.iabuf);
		prefered_ia_pd.address.len=16;
		iprintf("[dhclient] prefered_ia_pd.plen=%d, prefered_ia_pd.address.len=%d, prefered_ia_pd.address.iabuf=", prefered_ia_pd.plen, prefered_ia_pd.address.len);
		dumpv6addr(prefered_ia_pd.address.iabuf);
		iprintf("\n");
	}

	// terminate transaction
	mapi_end_transc(ipv6_tid);
	#endif
	//
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-r")) {
			...
			iprintf("[dhclient] release_mode, no_daemon, line %d\n", __LINE__);
#ifdef DHCPv6
			...
			if (local_family_set && local_family != AF_INET)
			{
				log_fatal("Client can only do v4 or v6, not "
					  "both.");
				iprintf("[dhclient] Client can only do v4 or v6, not "
					  "both.\n");
			}
			local_family_set = 1;
			local_family = AF_INET;

			iprintf("[dhclient] IPv4\n");
			...
			iprintf("[dhclient] IPv6\n");
		} else if (!strcmp(argv[i], "-dec")) {
			/* grace add "-dec" for decline */
			decline_mode = 1;
			no_daemon = 1;
#endif /* DHCPv6 */
		...
		/* hsubj add to set default required lease time */
			...
			if (++i == argc)
                                usage();
                        default_request_lease_time = atoi(argv[i]);
...
//add by michael to implementation the bridge mode
	...
	//hanlde signal	Add by Alpha 2010-11-11
	 signal(SIGUSR1, signal_handler);
        signal(SIGUSR2, signal_handler);
        signal(SIGTERM, signal_handler);

	/* first kill off any currently running client */
	if (release_mode || exit_mode || decline_mode) {
		FILE *pidfd;
		pid_t oldpid;
		long temp;
		int e;
		...
		#ifdef DHCLIENT_DEBUG
		iprintf("[dhclient.c main] oldpid = %d , line:%d\n",oldpid,__LINE__) ;
		#endif	// DHCLIENT_DEBUG
			...
			if (e != 0 && e != EOF) {
				if (oldpid) {
					...
					for (client = ip->client; client; client = client->next)
						...
						{
							#ifdef DHCLIENT_DEBUG
							iprintf("[dhclient.c main] call do_release , line:%d\n",__LINE__) ;
							#endif	// DHCLIENT_DEBUG
	...
	if (stateless) {
		if (release_mode || (wanted_ia_na > 0) ||
			...
			#ifdef DHCLIENT_DEBUG
			iprintf("[dhclient.c main] call usage() , line:%d\n",__LINE__) ;
			#endif	// DHCLIENT_DEBUG
		...
		for (ip = interfaces ; ip != NULL ; ip = ip->next) {
			for (client = ip->client ; client != NULL ;
				...
				if (release_mode) {
					...
					#ifdef DHCLIENT_DEBUG
					iprintf("[dhclient.c main] call start_release6() , line:%d\n",__LINE__) ;
					#endif	// DHCLIENT_DEBUG

					ret = start_release6(client);
					if(!ret)
						Nothing2Do = 0; /* 2011-04-01 If no client info, currently running client will exit. */
					continue;
				} else if (exit_mode) {
					unconfigure6(client, "STOP6");
					continue;
				} else if (decline_mode) {
					/* grace add decline */
				...
				{
					#ifdef DHCLIENT_DEBUG
					iprintf("[dhclient.c main] call start_init6(client) , line:%d\n",__LINE__) ;
					#endif	// DHCLIENT_DEBUG
...
//add by michael to implementation the bridge mode
			...
			for (client = ip->client ; client ;
				...
				{
					#ifdef DHCLIENT_DEBUG
					iprintf("[dhclient.c main] call do_release6() , line:%d\n",__LINE__) ;
					#endif	// DHCLIENT_DEBUG
	...
	if (release_mode) {
		...
		if (local_family == AF_INET6) {
			...
			{
				#ifdef DHCLIENT_DEBUG
				iprintf("[dhclient.c main] return 0 , line:%d\n",__LINE__) ;
				#endif	// DHCLIENT_DEBUG
				
				return 0;
			}
			if (Nothing2Do) /* 2011-04-01 If no client info, currently running client will exit. */
			{
				#ifdef DHCLIENT_DEBUG
				iprintf("[dhclient.c main] return 0 , line:%d\n",__LINE__) ;
				#endif	// DHCLIENT_DEBUG
...
static void usage()
{
	...
	log_error("Usage: dhclient %s %s",
		  ...
		  "[-1dvrx] [-nw] [-er] [-p <port>]",
#endif /* DHCPv6 */
		  "[-s server] [-ss select server]");
	log_error("                [-cf config-file] [-lf lease-file]%s",
		  "[-pf pid-file] [-e VAR=val]");
	log_error("                [-t lease-time]");
...
void run_stateless(int exit_mode)
{
	...
	iprintf("[run_stateless] exit_mode=%d\n", exit_mode);
...
{
	struct client_state *client = cpp;
	first_rebind = 1;	//reset flag in init state
	renew_requested_cnt = 0;
	...
	client -> first_sending = cur_time;
	//client -> interval = client -> config -> initial_interval;
	client -> interval = 0; // in order to set the retry algorithm for DT W724, see line 2345
	...
	struct packet *packet;
{
	...
	/* Add by fpan for W724 Spec at 20120522*/
...
#if 0	//don't random renewal time 2010-12-23
...
//				(((random() % client->new->renewal) + 3) / 4);
		client->new->renewal = client->new->renewal+((random() % client->new->renewal)*3/4);
	}
#endif
	...
	first_rebind = 1;	//reset flag when receive ACK packet
	renew_requested_cnt = 0;
	bind_lease (client);
}
	...
	struct packet *packet;
{
	...
	for (ap = packet -> interface -> client -> config -> reject_list;
		...
		if (addr_match(&packet->client_addr, &ap->match)) {
			...
			fp = fopen("/tmp/dhcreject_log","w");
			fprintf(fp,"%s",piaddr(packet->client_addr));
			fclose(fp);
	...
	struct packet *packet;
{
	...
	sprintf (obuf, "%s from %s:%s", name, packet->interface->name, piaddr (packet -> client_addr));
	...
	if(bridge_mode != 1)
	{
		/******************start***********************************/
		/*
		*	Add by Alpha to set the wan ipaddr to 127.0.0.1
		*	when wan ipaddr is conflict with lanip (IR-B0016094)
		*	2010-12-14
		*
		**********************************************************/
		/* FIXME: DHCP */
	...
	if (select_server) {
                ...
                if (oc && evaluate_option_cache(&data, (struct packet *)0,
                        ...
                        if (data.len > 3) {
                                if (strcmp(pretty_print_option(oc -> option,
                                        data.data, data.len, 1, 1), select_server)) {
                                        data_string_forget(&data, MDL);
                                        log_error("%s: This has been rejected. Expected DHCP server is %s",
                                                obuf, select_server);
                                        return;
                                }
                                if(!strcmp(packet->raw->file, "")) {
                                        log_info("%s: no provision-file option.", obuf);
                                        return;
                                }
                        } else {
                                data_string_forget(&data, MDL);
	...
	struct client_state *client;
{
	...
	memcpy (lease -> address.iabuf, &packet -> raw -> yiaddr,
		lease -> address.len);
	lease -> server_address.len = sizeof (packet -> raw -> siaddr);
	memcpy (lease -> server_address.iabuf, &packet -> raw -> siaddr,
		lease -> server_address.len);
	...
	void *cpp;
{
	...
	*/
	...
	if (increase) {
		if (!client->interval)
			client->interval = client->config->initial_interval;
		else if(client->interval == client->config->initial_interval)
			client->interval = 1;
		else if(client->interval >= 256)
			client->interval = 600;
		else
			client->interval = client->interval<<1;//2 * client->interval;
...
#include <sys/sysinfo.h>
		if(begin_flag == 0)
		{	//mark the first dhcp discovery time
			struct sysinfo info;
			sysinfo(&info);
			begin_time = info.uptime;
			begin_flag = 1;
		}
		else if(setnoip_flag == 0)
		{
			struct sysinfo info;
			sysinfo(&info);
			if(info.uptime - begin_time > 45)
			{
				//after 45s still get no ip, set the 169.254.*.* for router
				setnoip_flag = 1;
				script_init(client,"SWITCHNOIP",(struct string_list *)0);
				script_go(client);
			}
		}
	}
	...
	void *cpp;
{
	...
	(60 seconds or 4 tries if using timeout suggested in section   4.1)
	*********************************************************/
	if (client -> state == S_REQUESTING)
	{
	if (client -> interval<4)
		client -> interval = 4;//client -> config -> initial_interval + 1;
		else
			client -> interval = 2* client -> interval;//((random () >> 2) %    //  (2 * client -> interval));
	}
		...
		client -> interval =
			((client -> config -> backoff_cutoff / 2)
			 + ((random () >> 2) %
					client -> config -> backoff_cutoff));
*/
	/* If the backoff would take us to the rebind time, just set the
		   timeout to the rebind time. */
	/*********************************************************
	In rfc 2131 4.4.5
	...
	lease time (in REBINDING state), down to a minimum of 60 seconds,
	before retransmitting the DHCPREQUEST message.
	*********************************************************/
	if (client -> state == S_RENEWING)
	{
		// Increase the probability to pass cdrouter_dhcp_client_ext_3, don't send packet before rebind within 5 seconds.
		//else if (cur_time + client -> interval > client -> active -> rebind - 5)
		//	client -> interval =
		//		client -> active -> rebind - cur_time + 1;
		if (cur_time < client -> active -> rebind)
		{
			if (client -> active -> rebind - cur_time > 60)
				client -> interval = ((client -> active -> rebind - cur_time) / 2 > 60) ?
					(client -> active -> rebind - cur_time) / 2 : 60;
			else
				client -> interval = 60 + 1;
		}
		else
			client -> state = S_REBINDING;
	}
	else if (client -> state == S_REBINDING)
	{
		if (cur_time < client -> active -> expiry)
		{
			if (client -> active -> expiry - cur_time > 60)
				client -> interval = ((client -> active -> expiry - cur_time) / 2 > 60) ?
					(client -> active -> expiry - cur_time) / 2 : 60;
			else
				client -> interval = 60 + 1;
		}
		else
	...
	tv . tv_sec = cur_time + client -> interval;
	tv . tv_usec = 0;
	if ((client->state != S_REQUESTING && client->state != S_REBOOTING)
		&& client->active && first_rebind && tv.tv_sec > client->active->rebind)
	{
		tv . tv_sec = client->active->rebind;	//next request packet is rebind state packet
		first_rebind = 0;
	}
	...
	void *cpp;
{
	...
	if (fallback_interface)
		...
		result = send_packet (client -> interface, (struct packet *)0,
				      ...
				      from, &destination,
				      (struct hardware *)0);
	//add by michael to support bridge_mode at 20110216
	if(bridge_mode == 1)
	{
		begin_flag = 0;
		begin_time = 0;
	...
	struct client_lease *lease;
{
	...
	struct iaddr zero_rip;

	zero_rip.len = 4;
	memset(zero_rip.iabuf, 0, sizeof(zero_rip.iabuf));
	...
	struct client_lease *lease;
{
	...
	if (client -> state == S_BOUND ||
	    client -> state == S_RENEWING ||
	    client -> state == S_REBINDING|| client->state == S_REQUESTED) {
	...
	if (client -> state != S_BOUND &&
	    client -> state != S_RENEWING|| client->state != S_REQUESTED)
		client -> packet.giaddr = giaddr;
	else
	...
	struct client_lease *lease;
{
	...
	/*Fixed CRDC bug E3200-4(P2) :
	*  WAN DHCP client: Client IP Address in DHCPDECLINE is all zero,
	*  expected the declined IP address.
	*/
	memcpy(&(client->packet.ciaddr), &(lease->address.iabuf), sizeof(client->packet.ciaddr));
	...
	int makesure;
{
	...
	if (lease -> server_address.len)
		fprintf (leaseFile, "  server-address %s;\n",
			piaddr (lease -> server_address));
	if (errno) {
		++errors;
		errno = 0;
	}
	...
	struct client_lease *lease;
{
	...
	client_envadd (client,
		       prefix, "server_address", "%s", piaddr (lease -> server_address));
		    ...
		    const char *prefix, const char *name, const char *fmt, ...)
{
	...
	*s++ = '=';
	if (len >= sizeof spbuf) {
		...
		strcpy (s, spbuf);
	iprintf("[client_envadd] prefix=%s, name=%s, s=%s\n", prefix, name, s);
	val -> next = client -> env;
	client -> env = val;
	client -> envc++;
}
	...
	struct client_state *client;
{
	...
	if(decline_flag == 1)	//Add by Alpha to set the wan ipaddr to 127.0.0.1 when wan ipaddr is conflict with lanip (IR-B0016094)
	{
		decline_flag = 0;
		script_init (client, "RELEASE", (struct string_list *)0);
		script_go (client);
	}
	...
	cancel_timeout (state_reboot, client);
	client -> state = S_STOPPED;
	first_rebind = 1;	//reset flag when release
	renew_requested_cnt = 0;
