...
//add by michael to support bridge mode in dhcpc
int bridge_mode = 0;
const char *bridge_rcvif = NULL;
const char *bridge_sendif = NULL;
	...
	omapi_object_t *h;
{
	...
	if (bootp_packet_handler) {
		...
		if(bridge_mode == 1)
		{
			...
			for(tmp = interfaces; tmp; tmp=tmp->next)
			{
				if(tmp->name && bridge_sendif && !strcmp(tmp->name,bridge_sendif))
					ip = tmp;	
			}
		}
