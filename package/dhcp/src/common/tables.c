...
#endif
#if 1 /* 2010-11-23 ruby add for 6rd */
	{ "6rd-info", "BA",			&dhcp_universe, 212, 1 },
#endif
#if 0
	/* Not defined by RFC yet */
...
struct universe ccc_universe;
static struct option ccc_options[] = {
	{ "dhcp-server-1", "I",			&ccc_universe,   1, 1 },
	{ "dhcp-server-2", "I",			&ccc_universe,   2, 1 },
	{ "provision-server", "X",		&ccc_universe,   3, 1 },
	{ "as-backoff-retry", "LLL",		&ccc_universe,   4, 1 },
	{ "ap-backoff-retry", "LLL",		&ccc_universe,   5, 1 },
	{ "kerberos-realm", "D",		&ccc_universe,   6, 1 },
	{ "tgs-usage", "f",			&ccc_universe,   7, 1 },
	{ "provision-timer", "B",		&ccc_universe,   8, 1 },
	{ "security-ticket-invalidation", "S",	&ccc_universe,   9, 1 },
	{ NULL, NULL, NULL, 0, 0 }
};
...
static struct option vsio_options[] = {
	...
	{ "tftp-servers",	"6",			&vsio_universe,		     32,   1 },
	{ "config-file-name",	"t",			&vsio_universe,		     33,   1 },
	{ "syslog-servers",	"6",			&vsio_universe,		     34,   1 },
	{ "ip-pref",		"B",			&vsio_universe,		     39,   1 },
	{ "cablelabs-client-configuration", "Ecccv6.",
							&vsio_universe,		     2171,  1 },
	{ NULL, NULL, NULL, 0, 0 }
};
...
struct universe cccv6_universe;
static struct option cccv6_options[] = {
	{ "dhcp-server-1", "6",			&cccv6_universe,   1, 1 },
	{ "dhcp-server-2", "6",			&cccv6_universe,   2, 1 },
	{ "provision-server", "X",		&cccv6_universe,   3, 1 },
	{ "as-backoff-retry", "LLL",		&cccv6_universe,   4, 1 },
	{ "ap-backoff-retry", "LLL",		&cccv6_universe,   5, 1 },
	{ "kerberos-realm", "D",		&cccv6_universe,   6, 1 },
	{ "tgs-usage", "f",			&cccv6_universe,   7, 1 },
	{ "provision-timer", "B",		&cccv6_universe,   8, 1 },
	{ "security-ticket-invalidation", "S",	&cccv6_universe,   9, 1 },
	{ NULL, NULL, NULL, 0, 0 }
};
...
void initialize_common_option_spaces()
{
	 ...
	 * 13: ccc_universe (dhcpv4 CableLabs Client Configuration option)
	 * 14: more user-config
	 * 15: more user-config
	 * 16: more user-config
	 * 17: cccv6_universe (dhcpv6 CableLabs Client Configuration option)
	 */
	universe_max = 16;
	...
	/* Set up the CCC option universe... */
	ccc_universe.name = "ccc";
	ccc_universe.concat_duplicates = 0;
	ccc_universe.lookup_func = lookup_linked_option;
	ccc_universe.option_state_dereference =
		linked_option_state_dereference;
	ccc_universe.save_func = save_linked_option;
	ccc_universe.delete_func = delete_linked_option;
	ccc_universe.encapsulate = linked_option_space_encapsulate;
	ccc_universe.foreach = linked_option_space_foreach;
	ccc_universe.decode = ccc_universe_decode;
	ccc_universe.length_size = 1;
	ccc_universe.tag_size = 1;
	ccc_universe.get_tag = getUChar;
	ccc_universe.store_tag = putUChar;
	ccc_universe.get_length = getUChar;
	ccc_universe.store_length = putUChar;
	ccc_universe.site_code_min = 0;
	ccc_universe.end = 0;
	ccc_universe.index = universe_count++;
	code = DHO_CCC;
	ccc_universe.enc_opt = NULL;
	if (!option_code_hash_lookup(&ccc_universe.enc_opt,
				     dhcp_universe.code_hash, &code, 0, MDL))
		log_fatal("Unable to find CCC parent option (%s:%d).", MDL);
	universes [ccc_universe.index] = &ccc_universe;
	if (!option_name_new_hash(&ccc_universe.name_hash,
				  CCC_HASH_SIZE, MDL) ||
	    !option_code_new_hash(&ccc_universe.code_hash,
				  CCC_HASH_SIZE, MDL))
		log_fatal ("Can't allocate ccc option hash table.");
	for (i = 0 ; ccc_options[i].name ; i++) {
		option_code_hash_add(ccc_universe.code_hash,
				     &ccc_options[i].code, 0,
				     &ccc_options[i], MDL);
		option_name_hash_add(ccc_universe.name_hash,
				     ccc_options[i].name, 0,
				     &ccc_options[i], MDL);
	}
#if defined(REPORT_HASH_PERFORMANCE)
	log_info("CCC name hash: %s",
		 option_name_hash_report(ccc_universe.name_hash));
	log_info("CCC code hash: %s",
		 option_code_hash_report(ccc_universe.code_hash));
#endif
	...
	/* Set up the CCC6 option universe... */
	cccv6_universe.name = "cccv6";
	cccv6_universe.concat_duplicates = 0;
	cccv6_universe.lookup_func = lookup_hashed_option;
	cccv6_universe.option_state_dereference =
		hashed_option_state_dereference;
	cccv6_universe.save_func = save_hashed_option;
	cccv6_universe.delete_func = delete_hashed_option;
	cccv6_universe.encapsulate = hashed_option_space_encapsulate;
	cccv6_universe.foreach = hashed_option_space_foreach;
	cccv6_universe.decode = cccv6_universe_decode;
	cccv6_universe.length_size = 0;
	cccv6_universe.tag_size = 4;
	cccv6_universe.get_tag = getUChar;
	cccv6_universe.store_tag = putUChar;
	cccv6_universe.get_length = NULL;
	cccv6_universe.store_length = NULL;
	cccv6_universe.site_code_min = 0;
	cccv6_universe.end = 0x00;
	cccv6_universe.index = universe_count++;
	code = VSIO_CCCV6;
	//cccv6_universe.enc_opt = NULL;
	if (!option_code_hash_lookup(&cccv6_universe.enc_opt,
				     vsio_universe.code_hash, &code, 0, MDL))
		log_fatal("Unable to find CCCv6 parent option (%s:%d).", MDL);
	universes [cccv6_universe.index] = &cccv6_universe;
	if (!option_name_new_hash(&cccv6_universe.name_hash,
				  CCC_HASH_SIZE, MDL) ||
	    !option_code_new_hash(&cccv6_universe.code_hash,
				  CCC_HASH_SIZE, MDL))
		log_fatal ("Can't allocate ccc option hash table.");
	for (i = 0 ; cccv6_options[i].name ; i++) {
		option_code_hash_add(cccv6_universe.code_hash,
				     &cccv6_options[i].code, 0,
				     &cccv6_options[i], MDL);
		option_name_hash_add(cccv6_universe.name_hash,
				     cccv6_options[i].name, 0,
				     &cccv6_options[i], MDL);
	}
/*#if defined(REPORT_HASH_PERFORMANCE)
	log_info("CCC name hash: %s",
		 option_name_hash_report(cccv6_universe.name_hash));
	log_info("CCC code hash: %s",
		 option_code_hash_report(cccv6_universe.code_hash));
#endif*/
	...
	universe_hash_add(universe_hash, fqdn_universe.name, 0,
			  &fqdn_universe, MDL);
	universe_hash_add(universe_hash, ccc_universe.name, 0,
			  &ccc_universe, MDL);
	...
	universe_hash_add(universe_hash, isc6_universe.name, 0,
			  &isc6_universe, MDL);
	universe_hash_add(universe_hash, cccv6_universe.name, 0,
			  &cccv6_universe, MDL);
