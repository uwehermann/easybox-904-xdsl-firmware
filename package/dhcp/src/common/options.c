...
int ccc_universe_decode (struct option_state *options,
	const unsigned char *buffer, unsigned length, struct universe *universe)
{
	unsigned len, offset;
	unsigned code;
	struct option_cache *op = NULL, *nop = NULL;
	...
	for (offset = 0;
		...
		if (op != NULL && universe->concat_duplicates) {
			...
			option_cache_reference(&op->next, nop, MDL);
			option_cache_dereference(&nop, MDL);
		} else if (code == CCC_PROVISION_SERVER) {
			if (*(bp->data + offset) == 0) {
				int index;
				char zero[] = {0x01, 0x30, 0x01, 0x30, 0x01, 0x30, 0x01, 0x30, 0x00};

				// don't use hostname or 0.0.0.0
				if ((!memcmp(bp->data + offset + 1, zero, sizeof(zero)) &&
					len == 10) || *(bp->data + offset + 1) + 3 == len) {
					log_error("option %s: don't use hostname or 0.0.0.0.",
						option->name);
					return 0;
				}
				...
				for (index = 1; index < len - 1; index ++) {
					...
					// label length must less then 63
					if (*(bp->data + offset + index) > 63) {
						log_error("option %s: FQDN label length must less then 63.",
							option->name);
						return 0;
					}
					// label must between a(A) and z(Z) or be '-'
					label_end = *(bp->data + offset + index) + index;
					while (index < label_end) {
						index ++;
						if (!isalnum(*(bp->data + offset + index)) &&
							*(bp->data + offset + index) != '-') {
							log_error("option %s: FQDN label must between a(A) and z(Z) or be \'-\'.",
								option->name);
							return 0;
						}
					}
				}
			} else if (*(bp->data + offset) == 1) {
				// don't use 0.0.0.0
				if (len != 5 || *(bp->data + offset + 1) == 0 ||
					*(bp->data + offset + 1) == 255) {
					log_error("option %s: don't use 0.0.0.0.",
						option->name);
					return 0;
				}
			} else {
		...
		} else if (code == CCC_KERBEROS_REALM) {
#if 0
			int index;

			// don't use hostname
			if (*(bp->data + offset) + 2 == len) {
				log_error("option %s: don't use hostname.",
					option->name);
				return 0;
			}
...
#else
			char basic1[] = {0x05, 0x42, 0x41, 0x53, 0x49, 0x43, 0x01, 0x31, 0x00};

			// must use BASIC.1
			if (memcmp(bp->data + offset, basic1, sizeof(basic1))) {
				log_error("option %s: It is invalid, \"BASIC.1\" is expected only.",
					option->name);
				return 0;
			}
#endif
...
store_options6(char *buf, int buflen, 
	...
	unsigned char orobuf[64];
	unsigned char *pORO=NULL;
	int queryDNS = 0;
	/*end bruce add*/
...
int cccv6_universe_decode (struct option_state *options,
	const unsigned char *buffer, unsigned length, struct universe *universe)
{
	unsigned len, offset;
	unsigned code;
	...
	for (offset = 0;
		...
		if (op != NULL && universe->concat_duplicates) {
			...
			option_cache_reference(&op->next, nop, MDL);
			option_cache_dereference(&nop, MDL);
		} else if (code == CCCV6_PROVISION_SERVER) {
			if (*(bp->data + offset) == 0) {
				int index;
				char zero[] = {0x01, 0x30, 0x01, 0x30, 0x01, 0x30, 0x01, 0x30, 0x00};
		...
		} else if (code == CCCV6_KERBEROS_REALM) {
#if 0
			int index;
