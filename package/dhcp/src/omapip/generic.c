...
// When system time change, the lease table will error, so we use system uptime	2010-11-11
int get_time(struct timeval *tv)
{
#if 0 /* Forrest.Fei 2012.12.19 Use /proc/uptime to replace sysinfo for improving time precision. */
        struct sysinfo info;
        sysinfo(&info);
	...
	fp = fopen(UPTIME, "r");
	if(NULL == fp)
		log_fatal(UPTIME" open error\n");
	fscanf(fp, "%d.%d", &tv->tv_sec, &tv->tv_usec);
