...
long osSystem( char* sShellCmd )
{
	...
	//error("[osSystem] sShellCmd=%s, line %d\n", sShellCmd, __LINE__);
	if (system( sShellCmd ) == -1)
		return ARC_COM_ERROR;
 ...
 *
 * Note
 *      * only up to iLen-1 bytes are copied into sBuf if output size is larger than iLen
 *
 ******************************************************************************/
long osSystem_GetOutput ( char* sShellCmd, char* sBuf, int iBufSz )
{
	...
	if (strlen(sShellCmd) > (sizeof(sCmd)-20))
		return ARC_COM_ERROR;
	...
	{
		error("[osSystem_GetOutput] osSystem(%s) error 1\n", sCmd);
		return ARC_COM_ERROR;
	}

	if ( (pFile=fopen(sTmpFn,"r")) == 0 )
	{
		error("[osSystem_GetOutput] fopen() error\n");
		return ARC_COM_ERROR;
	...
	//error("[osSystem_GetOutput] lCnt=%d, line %d\n", lCnt, __LINE__);
