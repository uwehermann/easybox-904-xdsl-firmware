...
static struct fuse *mount_fuse(char *parsed_options)
{
	...
	if (fuse_opt_add_arg(&args, "") == -1)
		goto err;
#if !CACHEING
  #if 0 /* ctc, performance issue */
	if (fuse_opt_add_arg(&args, "-ouse_ino,kernel_cache,attr_timeout=0") == -1)
  #else
	if (fuse_opt_add_arg(&args, "-ouse_ino,noauto_cache,direct_io,attr_timeout=0") == -1)
  #endif
		goto err;
#else
...
static void setup_logging(char *parsed_options)
{
  ...
  #if 1 // ctc
	char sCmd[256];
	sprintf( sCmd, "ntfs_mtd.sh '%s' '%d.%d' '%s' &", opts.device, ctx->vol->major_ver, ctx->vol->minor_ver, ctx->vol->vol_name );
	system( sCmd );
  #endif
}
