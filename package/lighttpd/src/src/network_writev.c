...
#ifndef MMAP_WINDOWS_START
  #define MMAP_WINDOWS_START    (MAP_FAILED - 1)
#endif
...
#define GByte * 1024 MByte
			...
			if (c->file.mmap.start == MAP_FAILED ||
				...
				if  (c->file.mmap.start != MAP_FAILED)
				{
					if  (c->file.mmap.start != MMAP_WINDOWS_START) {
				...
				if (MAP_FAILED == (c->file.mmap.start = mmap(0, to_mmap, PROT_READ, MAP_SHARED, c->file.fd, c->file.mmap.offset))) {
					...
					//mmapFail = MAP_FAILED;
					c->file.mmap.start = MMAP_WINDOWS_START;
					//return -1;
				}
...
#endif
			//if ( mmapFail == MAP_FAILED )
			if ( c->file.mmap.start == MMAP_WINDOWS_START )
			{
			    //buffer_copy_string_len(c->mem, c->file.mmap.start, c->file.mmap.length);
			    buffer_prepare_copy(c->mem, toSend);
			    //lseek(c->file.fd, c->file.mmap.offset, SEEK_SET);
			    lseek(c->file.fd, abs_offset, SEEK_SET);
			    if (-1 == (toSend = read(c->file.fd, c->mem->ptr, toSend))) {
			    ...
			    //close(ifd);
			    //start = c->mem->ptr;
			    start = c->mem->ptr - (abs_offset - c->file.mmap.offset);
			...
			if (c->offset == c->file.length) {
				...
				if ( (c->file.mmap.start != MAP_FAILED) && (c->file.mmap.start != MMAP_WINDOWS_START) ) {
					munmap(c->file.mmap.start, c->file.mmap.length);
