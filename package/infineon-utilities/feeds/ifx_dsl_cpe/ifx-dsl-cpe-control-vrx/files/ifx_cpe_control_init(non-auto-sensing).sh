...
calc_xtse() {
   ...
   check=${#ADSL_MODE}
  select_adsl_mode=`cat $SYS_CONF | grep CONFIG_FW_XDSL_ANNEX_B | cut -d'=' -f2`
  echo $select_adsl_mode >/dev/console
  if [ $select_adsl_mode -eq 1 ]; then
	#if [ $CONFIG_FW_XDSL_ANNEX_B_1 -eq 1 ]; then
		ADSL_MODE="Annex_1_0_1_1_0_1_0_0_0_0_1_1_1_0_0"
		echo "CONFIG_FW_XDSL_ANNEX_B_* is on" > /dev/console
		if [ $CONFIG_FW_XDSL_ANNEX_J -eq 1 ]; then
			ADSL_MODE="Annex_1_0_1_1_0_1_0_0_0_0_1_1_1_1_1"
			echo "CONFIG_FW_XDSL_ANNEX_J is on" > /dev/console
		fi
	else
		ADSL_MODE="Annex_1_0_1_1_0_1_0_0_0_0_0_0_0_0_0"
		echo "CONFIG_FW_XDSL_ANNEX_B_* is not DEFINED !!" > /dev/console
		echo "Set to Annex A by default !!" > /dev/console
	fi
...
#            XDSL_MULTIMODE="-M2_0"
      ...
      if [ ${FW_FOUND} = 0 -o ${START_API} = 0 ]; then
         echo "ERROR: No firmware binary available within '${FW_DIR}'"
		...
		###########
         #${BIN_DIR}/dsl_cpe_pipe.sh llcs 0 1 81 81 81 1e116000 55 0 # disable V43 tone for LANTIQ testing
        ###########
