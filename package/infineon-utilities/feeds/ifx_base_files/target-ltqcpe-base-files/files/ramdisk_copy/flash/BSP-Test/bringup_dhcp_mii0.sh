...
# WAN DHCP IP configuration
PID="`cat /var/run/udhcpc${WAN_VLAN_ID}.pid 2> /dev/null`"
/etc/rc.d/checkpid $PID
status=$?
if [ "$status" -ne 0 ]; then
	echo "Interface - ($MII0_IF.$WAN_VLAN_ID) already configured!"
	echo "  Please modify the WAN Setting and try again"
	exit
else
	echo "Bringingup WAN Ethernet DHCP Connection on ($MII0_IF.$WAN_VLAN_ID)"
fi
...
/sbin/udhcpc -b -i $MII0_IF.$WAN_VLAN_ID -p /var/run/udhcpc${WAN_VLAN_ID}.pid -s /etc/rc.d/udhcpc.script 
sleep 1
