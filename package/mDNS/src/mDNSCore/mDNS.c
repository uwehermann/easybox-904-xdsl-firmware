...
//sowa++
	int j, k;
	char ipbuf[15]={0}, tmp[128], hostname[64], *t1, *t2;
//sowa++		
...
//sowa++	
		...
		if( (m->rec.r.resrec.rrtype == kDNSType_AAAA) || (m->rec.r.resrec.rrtype == kDNSType_A))
		{
			memset(cmdLine, 0, sizeof(cmdLine));
			memset(hostname, 0, sizeof(hostname));
			memset(tmp, 0, sizeof(tmp));
			strncpy(tmp, &m->rec.namestorage, strlen(&m->rec.namestorage));				

			t2 =strstr(tmp, "_afpovertcp");								
			t1 =strstr(tmp, "local");
			...
			if((t1 != 0) && (t2 == 0))
			{
				...
				sprintf(ipbuf, "%d.%d.%d.%d",
						(srcaddr->ip.v4.NotAnInteger) >> 24, 
						((srcaddr->ip.v4.NotAnInteger) >> 16) & 0x00FF, 
						((srcaddr->ip.v4.NotAnInteger) >> 8) & 0x0000FF, 
						(srcaddr->ip.v4.NotAnInteger) & 0x000000FF);
#if 0 //debug use
				printf("@@@ [%s(%d)]: mDNS get IP:%s, length=%d, Hostname:%s, ifname:%s\n",
					   __FUNCTION__, __LINE__, ipbuf, strlen(hostname), hostname, m->HostInterfaces->ifname);
#endif				
				sprintf(cmdLine, "ccfg_cli -f /tmp/mDNS_lease.cfg set hostname@%s=%s", ipbuf, hostname);
				system(cmdLine);
			}
		}
//sowa--	
